"""
titan_hcl/utils/system_sensor.py — CPU + thermal + circadian rich signals.

Produces three independent rich sources for outer_body V6 5DT composites:
- CPU load (normalized per-core, 1.0 = saturated)
- CPU thermal (best-effort via /sys/class/thermal; CPU load-average proxy when unavailable)
    ...
    try:
        ...
        if not _thermal_unavailable_logged:
            logger.warning(
                ...
                "falling back to CPU load-average as a thermal-load proxy "
                "(heat ∝ work). Expected on VPS without thermal passthrough."
            )
            _thermal_unavailable_logged = True
        # Heat is proportional to work: with no hardware sensor, the 1-min
        # load-average (normalized per core) is a faithful, varying
        # thermal-load proxy — and naturally smoothed, mirroring thermal
        # mass (temperature lags instantaneous utilization). Replaces the
        # flat 0.5 non-signal that left cpu_thermal-gated dims structurally
        # frozen on sensorless VPS: outer_spirit SAT[11] environmental_adaptation
        # (gate cpu_thermal>0.6 never fired) and inner_mind[9] inner_smell
        # (ambient stddev of cpu_thermal+circadian collapsed to ~0). The
        # >0.6 gate + §23.9 formulas are unchanged — they now see a live signal.
        normalized = get_cpu_load()
