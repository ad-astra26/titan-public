...
Phase flow (mirrors PLAN §architecture):
  ...
  3. SHADOW BOOT — spawn `titan_hcl.py --server --shadow-port N
...
class HealthCriteria:
    ...
    critical_modules: tuple[str, ...] = (
        # D-SPEC-116: spirit_worker retired → cognitive_worker is the Phase C
        # consciousness-loop host (body/mind = Rust-fed L1 state writers).
        "cognitive_worker", "body", "mind",  # consciousness loop (L1 Trinity + L2 engines)
    ...
    #: D-SPEC-116: spirit_worker retired → track cognitive_worker.uptime (the
    #: Phase C consciousness host that replaced it; autostart + always-running,
    #: uptime increments monotonically — a reliable post-swap liveness probe).
    smoke_field_path: str = "data.guardian.cognitive_worker.uptime"
...
# shadow_data_dir hardlink-break step (titan_hcl/core/shadow_data_dir.py
...
def _phase_shadow_boot(
    ...
    *,
    ...
    # Build command. We invoke titan_hcl.py directly via the same Python.
    project_root = Path(__file__).resolve().parents[2]
    cmd = [
        "python", "-u",
        str(project_root / "scripts" / "titan_hcl.py"),
        "--server",
    ...
    # D-SPEC-116: "spirit" → "cognitive_worker" (Phase C consciousness host).
    b2_1_critical = (("cognitive_worker", "imw", "api") if api_separated
                     else ("cognitive_worker", "imw"))
    health_criteria = (
        HealthCriteria(
...
def _phase_shutdown_old(
    ...
    when our parent (titan_hcl.py) catches the new kernel's
...
def orchestrate_shadow_swap(
    ...
    *,
    ...
    # 5-min cron skips its duplicate-titan_hcl kill heuristic during
    ...
    try:
        ...
        recheck_critical = (  # D-SPEC-116: "spirit" → "cognitive_worker"
            ("cognitive_worker", "imw", "api") if api_separated_recheck
            else ("cognitive_worker", "imw")
        )
        recheck_criteria = (
            HealthCriteria(
