...
class TieredMemoryGraph:
    ...
    def __init__(self, config: dict = None, bus_emit=None):
        ...
        from titan_hcl.core.direct_memory import (
        ...
        # construction REMOVED. TitanCognify now routes entity-extraction
        # LLM calls through POST /v4/llm-distill so all LLM traffic appears
        # in llm_state.bin centrally. api_base + internal_key are read from
        # config and threaded into the engine; no provider object held here.
        _api_port = int(config.get("api", {}).get("port", 7777))
        _api_base = f"http://127.0.0.1:{_api_port}"
        _internal_key = config.get("api", {}).get("internal_key", "") or ""
        self._llm_client = None  # legacy attr; consumers should use cognify_engine
        self._cognify_engine = TitanCognify(
            llm_client=None,
            graph=self._graph,
            api_base=_api_base,
