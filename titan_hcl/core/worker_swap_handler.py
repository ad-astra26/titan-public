...
def on_bus_adopt_ack(state: SwapHandlerState, msg: dict) -> None:
    ...
    # Closed BUG-PHASE-B-OLD-WORKER-SELF-SIGTERM-ON-NAME-ALIASED-ACK-20260514:
    # the prior check `state._adopt_rid is not None and incoming_rid != state._adopt_rid`
    # treated `_adopt_rid is None` as a wildcard-accept. A worker that had
    # NEVER requested adoption (e.g., the OLD owner of a multi-name `name`
    # alias receiving the broker's fanout of Phase B's BUS_WORKER_ADOPT_ACK
    # destined for a sibling NEW pid) would proceed past this guard, see
    # status != "adopted", and self-SIGTERM on an ACK it never requested.
    # Correct semantics per SPEC §8.4: ONLY the worker that emitted
    # ADOPTION_REQUEST owns the matching ACK. Workers with `_adopt_rid is
    # None` MUST ignore.
    if state._adopt_rid is None:
        logger.debug(
            "[%s] BUS_WORKER_ADOPT_ACK ignored — this worker never requested "
            "adoption (incoming rid=%s); name-aliased fanout from a sibling "
            "worker is expected behavior under Phase B reload",
            state.name, (incoming_rid or "")[:8],
        )
        return
    if incoming_rid != state._adopt_rid:
        logger.debug(
