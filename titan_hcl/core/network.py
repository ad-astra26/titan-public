...
class HybridNetworkClient:
    ...
    def __init__(self, mood_engine=None, config: dict = None):
        ...
        # Last-known balance + reachability — updated by the kernel
        # balance-publisher loop (_publish_balance_once) on every successful
        # fetch. Exposed via the `balance` + `is_available` properties below
        # so the network_state_publisher (which reads getattr(network,
        # "balance"/"is_available")) writes the real values into
        # network_state.bin. Without these, the slot always reported
        # balance_sol=0 + network_available=False regardless of live RPC.
        self._balance_cache = None  # float SOL, or None until first fetch
    ...
    def balance(self) -> float:
        """Last-known SOL balance (lamports/1e9). Populated by the kernel
        balance-publisher loop on each successful fetch. 0.0 until first
        successful fetch. Consumed by network_state_publisher → network_state.bin
        → /status.sol_balance + Observatory."""
        return float(self._balance_cache) if self._balance_cache is not None else 0.0

    @property
    def is_available(self) -> bool:
        """True when a keypair is loaded AND the last balance fetch succeeded.
        Drives network_state.bin.network_available → /health RPC_CONNECTIVITY.
        Decoupled from wallet-loaded (pubkey) so a transient RPC failure
        reports RPC degraded without falsely marking the wallet unloaded."""
        return bool(self._pubkey is not None and self._network_available)
    ...
    async def get_account_info(self, pubkey_str: str) -> dict | None:
        ...
        """
        try:
            from titan_hcl.utils.solana_client import parse_pubkey, decode_zk_account_data
