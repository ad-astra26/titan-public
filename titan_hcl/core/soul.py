...
class SovereignSoul:
    ...
    def __init__(self, wallet_path: str, network_client, config: dict = None):
        ...
        # Mainnet Lifecycle Wiring rFP (2026-04-20) — superseded by
        # rFP_titan_hcl_l2_separation_strategy §4.J + D-SPEC-51 (SPEC
        # v1.7.2, 2026-05-14). The legacy in-process `_metabolism`
        # reference + `set_metabolism()` reverse-injection are RETIRED
        # (Maker-locked 2026-05-14 "Replace with SHM read from soul").
        # Soul now constructs its own `MetabolismShmReader` for sub-ms
        # tier + feature-flag reads. NFT mint gating in `_check_nft_gate`
        # uses local TIER_FEATURES lookup via the reader — no bus
        # roundtrip needed for the kernel-side gate decision.
        self._metabolism = None  # legacy attr kept None for back-compat
                                  # (set_metabolism() is a no-op below)
        try:
            from titan_hcl.proxies.metabolism_proxy import MetabolismShmReader
            self._metabolism_reader = MetabolismShmReader()
        except Exception as _mb_err:
            # Defensive: if the reader can't initialize (e.g., SHM root
            # not yet created during very early boot), fall back to None.
            # _check_nft_gate fail-opens when reader is None — matches
            # pre-extraction behavior when `_metabolism` was None.
            logger.debug(
                "[Soul] MetabolismShmReader init deferred: %s", _mb_err)
            self._metabolism_reader = None
    ...
    # -------------------------------------------------------------------------
    # Public read surface (consumed by SoulStatePublisher → soul_state.bin →
    # /health MAKER_AUTH + StateAccessor.soul). These expose the private
    # `_maker_pubkey` / `_nft_address` / `_directives_cache` attrs that the
    # publisher's getattr(soul, "maker_pubkey"/"nft_address"/"active_directives")
    # reads expected — without them the publisher silently wrote "" / [] and
    # /health reported MAKER_AUTH=DEGRADED despite a valid config maker_pubkey.
    # -------------------------------------------------------------------------
    @property
    def maker_pubkey(self) -> str:
        return str(self._maker_pubkey) if self._maker_pubkey else ""

    @property
    def nft_address(self) -> str:
        return str(self._nft_address) if self._nft_address else ""

    @property
    def active_directives(self) -> list:
        return list(self._directives_cache or [])
    ...
    def set_metabolism(self, metabolism) -> None:
        """No-op shim retained for back-compat.

        Per rFP_titan_hcl_l2_separation_strategy §4.J + D-SPEC-51 (SPEC
        v1.7.2, 2026-05-14), Soul no longer holds a MetabolismController
        reference. The legacy `plugin.py:_wire_metabolism` injection site
        is gone; Soul reads metabolism state from SHM via
        `self._metabolism_reader` (constructed in __init__).

        This method stays as a no-op for any callers that haven't been
        updated yet — calling it has no effect but does not crash.
        """
        return
    ...
    def _check_nft_gate(self, op_name: str) -> Optional[str]:
        ...
        Per rFP_titan_hcl_l2_separation_strategy §4.J (SPEC v1.7.2 / D-SPEC-51,
        Maker-locked "Replace with SHM read from soul"): reads tier from
        metabolism_state.bin SHM and checks the feature flag locally
        against the canonical TIER_FEATURES dict (sub-ms; no bus roundtrip).
        Authoritative ring-buffer-tracked gate decisions still flow
        through `MetabolismProxy.evaluate_gate` work-RPC at sites that
        want the audit (memo_inscribe + dashboard).
        """
        reader = getattr(self, "_metabolism_reader", None)
        if reader is None:
            return None  # fail-open when reader unavailable (early boot)
        try:
            if reader.gates_enforced and not reader.can_use_feature("nfts"):
    ...
    async def verify_nft_ownership(self) -> bool:
        ...
        try:
            from titan_hcl.utils.solana_client import fetch_mpl_core_asset
