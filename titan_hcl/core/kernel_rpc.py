...
# moved to titan_hcl/core/_frame.py in Phase B.2 (commit C1) so both
...
class KernelRPCServer:
    ...
    def _maybe_await(self, result: Any) -> Any:
        ...
        if self._kernel_loop is not None and self._kernel_loop.is_running():
            ...
            # 2026-05-11 — bumped 30.0 → 180.0 to support /chat + /v4/pitch-chat
            # cold-start round-trips. plugin.run_chat() over kernel_rpc invokes
            # the full persona pipeline (memory recall + 671B-parameter LLM
            # inference + post-hooks) which legitimately takes 6-90s, with
            # T2's cold-start observed at 32s (just over the previous 30s
            # cap → TimeoutError surfacing in pitch_chat as decline_reason=
            # "exception"). 180s mirrors the in-endpoint timeout convention
            # used by chat.py + pitch_chat.py. All faster RPC calls are
            # unaffected (they complete well under 30s and return immediately).
