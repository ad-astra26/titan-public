...
class TitanKnowledgeGraph:
    ...
    def spine_self_recall(self) -> dict:
        """Read the Self hub in one hop (the G5 core; P4 recall builds on this):
        the linked self-Engrams (newest-first) + skills. Returns
        {"engrams": [...], "skills": [...]}."""
        from titan_hcl.synthesis.kuzu_spine_schema import SELF_NODE_ID
        ...
        try:
            ...
            qr = self._conn.execute(
                ...
                "RETURN p.skill_id, p.name", {"sid": SELF_NODE_ID})
            while qr.has_next():
                r = qr.get_next()
                out["skills"].append({"skill_id": r[0], "name": r[1]})
        except Exception as e:
