...
class TitanKnowledgeGraph:
    ...
    # ── SELF hub (RFP_titan_authored_soul_diary §7.P3a / INV-SD-16) ──────
    # The `Self` singleton links all Titan-about-himself data so recall is one
    # hop: SELF_HAS_ENGRAM (diary + self-engrams) + SELF_HAS_SKILL (→Production).
    # The outward-expression rels (SELF_HAS_EXPRESSION) + the Persona node are a
    # DEFERRED later step (persona mechanic undesigned — RFP frontmatter).

    def spine_ensure_self_node(self) -> bool:
        """MERGE-equivalent: ensure the singleton `Self` hub exists (idempotent;
        one Kuzu graph = one Titan). Returns True on success."""
        from titan_hcl.synthesis.kuzu_spine_schema import SELF_NODE_ID
        try:
            qr = self._conn.execute(
                "MATCH (s:Self {id: $id}) RETURN COUNT(s)", {"id": SELF_NODE_ID})
            if qr.has_next() and qr.get_next()[0] > 0:
                return True
            self._conn.execute(
                "CREATE (s:Self {id: $id, created_at: $ts})",
                {"id": SELF_NODE_ID, "ts": time.time()})
            logger.info("[KnowledgeGraph] created Self hub node (id=%s)", SELF_NODE_ID)
            return True
        except Exception as e:
            logger.warning("[KnowledgeGraph] spine_ensure_self_node failed: %s", e)
            return False

    def _spine_link_self(self, rel: str, dst_table: str, dst_key: str,
                         dst_val: str) -> bool:
        """Idempotently link `Self` →[rel]→ a dst node (existence-checked CREATE,
        since Kuzu's CREATE on a missing MATCH silently no-ops). `rel`/`dst_table`
        /`dst_key` are fixed internal constants (never user input). Returns True
        if linked or already-linked, False if the dst node is missing / on error."""
        from titan_hcl.synthesis.kuzu_spine_schema import SELF_NODE_ID
        try:
            qr = self._conn.execute(
                f"MATCH (n:{dst_table} {{{dst_key}: $v}}) RETURN COUNT(n)",
                {"v": dst_val})
            if not (qr.has_next() and qr.get_next()[0] > 0):
                return False  # dst node absent
            qr = self._conn.execute(
                f"MATCH (s:Self {{id: $sid}})-[r:{rel}]->"
                f"(n:{dst_table} {{{dst_key}: $v}}) RETURN COUNT(r)",
                {"sid": SELF_NODE_ID, "v": dst_val})
            if qr.has_next() and qr.get_next()[0] > 0:
                return True  # already linked — idempotent
            self._conn.execute(
                f"MATCH (s:Self {{id: $sid}}), (n:{dst_table} {{{dst_key}: $v}}) "
                f"CREATE (s)-[:{rel}]->(n)", {"sid": SELF_NODE_ID, "v": dst_val})
            return True
        except Exception as e:
            logger.warning(
                "[KnowledgeGraph] _spine_link_self(%s→%s=%s) failed: %s",
                rel, dst_table, dst_val, e)
            return False

    def spine_link_self_engram(self, concept_id: str, version: int) -> bool:
        """Link a self-Engram (diary entry / self-about engram) to the Self hub
        (SELF_HAS_ENGRAM). Idempotent; ensures the hub exists first."""
        self.spine_ensure_self_node()
        return self._spine_link_self(
            "SELF_HAS_ENGRAM", "Engram", "pk", self._spine_pk(concept_id, version))

    def spine_link_self_skill(self, skill_id: str) -> bool:
        """Link a Production skill to the Self hub (SELF_HAS_SKILL). Idempotent."""
        self.spine_ensure_self_node()
    ...
    def spine_backfill_self_links(self) -> dict:
        ...
        from boot (not only newly-consolidated ones). Cheap; safe every boot.
        Returns {"engrams": n, "skills": n} (now-linked counts)."""
        out = {"engrams": 0, "skills": 0}
        try:
            qr = self._conn.execute(
                "MATCH (c:Engram) WHERE c.domain_hint = 'self' "
                "RETURN c.concept_id, c.version")
            engrams = []
            while qr.has_next():
                r = qr.get_next()
                engrams.append((r[0], int(r[1])))
            for cid, ver in engrams:
                if self.spine_link_self_engram(cid, ver):
                    out["engrams"] += 1
            qr = self._conn.execute("MATCH (p:Production) RETURN p.skill_id")
            skills = []
            while qr.has_next():
                skills.append(qr.get_next()[0])
            for sk in skills:
                if self.spine_link_self_skill(sk):
                    out["skills"] += 1
            if out["engrams"] or out["skills"]:
                logger.info("[KnowledgeGraph] Self-hub backfill: %d engram(s) + "
                            "%d skill(s) linked", out["engrams"], out["skills"])
        except Exception as e:
            logger.warning("[KnowledgeGraph] spine_backfill_self_links failed: %s", e)
        return out

    def spine_self_recall(self) -> dict:
        """Read the Self hub in one hop (RFP §7.P3a/P4 — the G5 core + the P4
        self-recall traversal): the linked self-Engrams (newest-first) + skills.
        Each engram carries `anchor_tx` (so EngineRecall can deref its content)
        + `groundedness` (ranking); each skill carries `utility_score`. Returns
        {"engrams": [...], "skills": [...]}."""
        from titan_hcl.synthesis.kuzu_spine_schema import SELF_NODE_ID
        out: dict = {"engrams": [], "skills": []}
        try:
            qr = self._conn.execute(
                "MATCH (s:Self {id: $sid})-[:SELF_HAS_ENGRAM]->(c:Engram) "
                "RETURN c.concept_id, c.version, c.name, c.domain_hint, "
                "c.anchor_tx, c.groundedness "
                "ORDER BY c.created_at DESC", {"sid": SELF_NODE_ID})
            while qr.has_next():
                r = qr.get_next()
                out["engrams"].append({
                    "concept_id": r[0], "version": int(r[1]),
                    "name": r[2], "domain_hint": r[3],
                    "anchor_tx": r[4] or "", "groundedness": float(r[5] or 0.0)})
            qr = self._conn.execute(
                "MATCH (s:Self {id: $sid})-[:SELF_HAS_SKILL]->(p:Production) "
                "RETURN p.skill_id, p.name, p.utility_score", {"sid": SELF_NODE_ID})
            while qr.has_next():
                r = qr.get_next()
                out["skills"].append({
                    "skill_id": r[0], "name": r[1],
                    "utility_score": float(r[2] or 0.0)})
        except Exception as e:
            logger.warning("[KnowledgeGraph] spine_self_recall failed: %s", e)
        return out
