...
When the Guardian-supervised parent (titan_hcl) is killed gracefully (SIGTERM),
...
Related: titan_hcl/guardian.py:_module_wrapper (call site);
scripts/titan_hcl.py:512 (existing parent SIGTERM handler — unchanged
by this module; complements it for the OOM/SIGKILL case it cannot cover);
titan_hcl/core/worker_swap_handler.py (B.2.1 supervision-transfer
