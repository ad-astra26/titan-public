...
class RLProxy:
    ...
    def __init__(self, bus: DivineBus, guardian: Guardian):
        ...
        try:
            ...
            from titan_hcl.bus import SAGE_STATS as _SAGE_STATS
    ...
    def _ensure_started(self) -> None:
        ...
        if ensure_started_async_safe(
            self._guardian, "recorder", id(self), proxy_label="RLProxy"
        ):
            self._started = True
    ...
    def refresh_stats(self) -> dict:
        ...
        try:
            ...
            self._track_fallback("recorder_state",
                                 f"read_raised:{type(e).__name__}")
            raw = None
        if raw is None:
            self._track_fallback("recorder_state", "shm_unavailable")
            return dict(self._stats_cache)
        try:
    ...
    def record_transition(self, observation, action, reward, next_obs=None, done=False) -> int:
        ...
        publish helper in titan_hcl/__init__.py."""
        self._ensure_started()
        reply = self._bus.request(
            "rl_proxy", "recorder",
            {
