...
Loads ``titan_hcl/config.toml`` (repo-tracked, non-secret values) and
...
def load_titan_config(force_reload: bool = False,
    ...
    if not layer2_path.exists():
        _LOG.error("[config_loader] Base config not found at %s", layer2_path)
        if use_cache:
            _cache = {}
        return {}
    ...
    try:
        ...
        _LOG.error("[config_loader] Failed to parse %s: %s", layer2_path, e)
        if use_cache:
            _cache = {}
        return {}
