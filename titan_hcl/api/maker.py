...
async def x_force_post(request: Request):
    ...
    try:
        # Import constant from module directly — plugin.bus is an
        # _RPCRemoteRef in the api subprocess, so attr access on it
        # returns more remote refs that msgpack can't serialize. The
        # constant is a plain string and lives in titan_hcl.bus.
        from titan_hcl.bus import make_msg, X_FORCE_POST
        # D-SPEC-66 v1.11.0 PLAN §1.5 — dst flipped "spirit" → "social"
        # to route to the Phase C live X_FORCE_POST subscriber in
        # social_worker. Was previously landing at dead handler in
        # spirit_worker.py:7995 (heartbeat-stub since fleet-wide
        # Phase C 2026-05-14 → Maker force-post was off-air).
        bus.publish(make_msg(
            X_FORCE_POST, "maker_api", "social", {
                "topic": topic,
                "text_hint": text_hint,
...
async def trigger_resurrection(request: Request):
    ...
    # Limbo guard. NOTE (v1.47.0 / D-SPEC-109): the legacy
    # `from titan_hcl import TitanHCL; TitanHCL._limbo_mode` read was
    # retired with the V2 in-process parent. The live limbo signal
    # (`kernel.limbo_mode`) is NOT yet surfaced to the api subprocess — it is
    # hardcoded False here (state_mapping.py / dashboard.py), so this read
    # preserves the prior always-"not in limbo" behavior without the dead
    # class dependency. Surfacing live kernel limbo to the api is a separate
    # follow-up; `scripts/resurrection.py` remains the on-disk limbo authority.
    if not bool(getattr(plugin, "_limbo_mode", False)):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # disk by sovereignty_worker; next boot carries the incremented
            # great_cycle count.
            #
            ...
            try:
                ...
                if bus_inst is not None:
                    ...
                    bus_inst.publish(make_msg(
                        SOVEREIGNTY_INCREMENT_GREAT_CYCLE, "maker_api",
                        "sovereignty",
                        {"ts": _time.time(), "source": "resurrection"},
                    ))
                    logger.info(
                        "[Maker] SOVEREIGNTY_INCREMENT_GREAT_CYCLE emitted "
                        "(sovereignty_worker increments + persists JSON)")
                else:
                    logger.warning(
                        "[Maker] plugin.bus unavailable — "
                        "SOVEREIGNTY_INCREMENT_GREAT_CYCLE not emitted")
            except Exception as _sgce:
                logger.warning(
                    "[Maker] SOVEREIGNTY_INCREMENT_GREAT_CYCLE emit failed: %s",
                    _sgce)
...
async def trigger_shadow_swap(request: Request):
    ...
    try:
        ...
        logger.warning("[Maker] config load failed reading maker_full_stop: %s "
                       "— defaulting full_stop=True (safe).", cfg_err)
    if full_stop:
        logger.warning(
