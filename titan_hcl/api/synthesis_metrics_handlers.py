...
# ── POST — §7.B (B.3) per-turn user/Maker rating (non-verifiable lane) ────
...
async def post_v6_synthesis_turn_feedback(request: Request):
    ...
    keyed by `reasoning_id`. Per-lane: stars5 → (score−3)/2 ∈ [−1,1]; research3
    (1=wrong/2=good/3=excellent) → score−2 ∈ {−1,0,+1}. Maker (user_id=="maker")
    → a LARGER delta (maker_reward_weight in self_learning) AND a graphed
    `MakerAssessment` bond record under `Self`; an ordinary user is reward-only.
    A later/Maker rating SUPERSEDES a prior judge reward via the corrective-delta.

    Body: {reasoning_id, score, scale: "stars5"|"research3", goal_class?, turn_summary?}.
    Fire-and-forget. Returns {ok, accepted, reward, source}."""
    try:
        body = await request.json()
    except Exception:
        body = {}
    reasoning_id = body.get("reasoning_id")
    scale = body.get("scale", "stars5")
    ...
    source = "maker" if is_maker else "user"
    rid = reasoning_id.strip()
    ...
    try:
        ...
        if titan_state is None:
            return {"ok": False, "error": "titan_state_unavailable"}
        # The reward → self_learning (joins the stashed decision + trains, applying
        # the source weight + corrective-delta in _handle_reward).
        titan_state.bus.publish(make_msg(
            bus.SELF_LEARN_REWARD, "dashboard", "self_learning", {
                "parent_tool_call_tx": rid,
                "reward": reward,
                "source": source,
                "goal_class": str(body.get("goal_class", "") or ""),
            }))
        # Maker only — the Maker↔Titan bond: a graphed MakerAssessment node under Self.
        if is_maker:
            titan_state.bus.publish(make_msg(
                bus.MAKER_ASSESSMENT_RECORD, "dashboard", "synthesis", {
                    "reasoning_id": rid, "score": score, "scale": scale,
                    "reward": reward,
                    "turn_summary": str(body.get("turn_summary", "") or "")[:280],
                    "ts": time.time(),
                }))
    except Exception as e:
        logger.warning("[synthesis_metrics_handlers] turn_feedback publish failed: %s", e)
        return {"ok": False, "error": str(e)}
    return {"ok": True, "accepted": True, "reward": reward, "source": source}
