...
async def witness_tail(
    ...
    # Blocking I/O wrapped in to_thread per directive
    # `feedback_observatory_db_nonblocking.md` — observatory.db can be
    # multi-GB on long-running Titans (T1 ~4.2 GB as of 2026-05-26), so
    # the sqlite3 query MUST NOT block the asyncio event loop or the
    # entire API stalls until the SELECT scans / index-seeks. Same
    # to_thread pattern dashboard.py:3998 uses for MeditationStateReader.
    events, meditation = await asyncio.gather(
        asyncio.to_thread(
            _read_event_tail,
            _WITNESS_TAIL_EVENT_TYPES,
            max(1, min(events_limit, _TAIL_HARD_MAX)),
        ),
        asyncio.to_thread(_read_last_meditation, request),
    )
    cgn = _read_cgn_snapshot(request)  # SHM-direct, sub-millisecond, fine on loop
...
async def thinking_tail(
    ...
    """
    _validate_pitch_token_or_404(x_pitch_token)
    # to_thread per feedback_observatory_db_nonblocking.md (see witness-tail
    # above for the full reasoning — same DB, same nonblocking invariant).
    events = await asyncio.to_thread(
        _read_event_tail,
        _THINKING_TAIL_EVENT_TYPES,
        max(1, min(limit, _TAIL_HARD_MAX)),
    )
...
async def list_pitch_sessions(limit: int = 50) -> JSONResponse:
    ...
    no DB or bus involvement. Wrapped in to_thread because each JSONL is
    opened and scanned line-by-line — at limit=500 that's up to 500 file
    opens which can stall the event loop on slow disk.
    """
    sessions = await asyncio.to_thread(_scan_sessions, max(1, min(limit, 500)))
...
async def get_pitch_session(thread_id: str) -> JSONResponse:
    ...
    # File-system read off the event loop (same reasoning as list_pitch_sessions).
    lines = await asyncio.to_thread(_read_session_lines, safe)
