"""
titan_hcl/api/state_mapping.py — canonical map from legacy `plugin.X.Y`
...
aliases like `request.app.state.titan_hcl`) and looks up the chain
...
SPECIAL_PATTERNS: list[tuple[tuple[str, ...], str]] = [
    ...
    # Plugin-private attrs — Phase D D-SPEC-82 retired the bus-cache that
    # used to source these. Codemod now produces literal-default reads;
    # endpoints that need live values must read SHM-direct per Preamble
    # G18 (the residual reads matched the empty-cache behavior under
    # Phase A.4 G18 neutralization).
    (("plugin", "_limbo_mode"), "False"),
    (("plugin", "_current_user_id"), '""'),
    (("plugin", "_pending_self_composed"), "None"),
    (("plugin", "_pending_self_composed_confidence"), "0.0"),
    ...
    (("plugin", "_interface_advisor"), "None"),
