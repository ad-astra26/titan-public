"""
titan_hcl/api/command_sender.py — fire-and-forget command dispatcher
...
class CommandSender:
    ...
    def force_dream(self, reason: str = "admin_force", source: str = "admin") -> str:
        """Admin / maker path to force a dream cycle onset (testing / maintenance
        / inspection). Post-§4.I D8-3 cleanup (2026-05-15): dst flipped
        "spirit" → "cognitive" since cognitive_worker now owns DreamingEngine
        via InnerTrinityCoordinator (spirit_worker is heartbeat-stub under
        Phase C). cognitive_worker subscriber calls
        coordinator.dreaming.request_dream(reason) → next check_transition
        AWAKE branch returns BEGIN_DREAMING (bypasses wake_inertia + drive
        gate, matching FORCE SLEEP semantics).
        """
        return self.publish(
            "FORCE_DREAM_REQUEST", "cognitive_worker",
