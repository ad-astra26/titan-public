...
def knowledge_worker_main(recv_queue, send_queue, name: str, config: dict) -> None:
    ...
    def _kp_alert_callback(_kind: str, _backend: str, _ctx: dict) -> None:
        ...
        if not _telegram_alerts_enabled:
            return
        try:
            from titan_hcl.utils.maker_notify import notify_maker
    ...
    try:
        from titan_hcl.logic.meta_cgn import EdgeDetector as _P10EdgeDet
                    ...
                    if findings:
                        ...
                        try:
                            if _p10_edge_detector is not None and \
                                ...
                                # §5) — a first concept grounding is a knowledge
                                # experience. cognitive_worker enriches + records.
                                from titan_hcl.bus import (
                                    emit_experience_record)
                                emit_experience_record(
                                    send_queue, "knowledge",
                                    domain="knowledge",
                                    action_taken=f"ground:{str(topic)[:60]}",
                                    outcome_score=min(1.0, max(0.0, float(quality))),
                                    context={"topic": str(topic)[:60],
                                             "requestor": str(requestor)[:40],
                                             "source": "concept_grounded"},
                                    coalesce_key="knowledge:ground",
                                )
                        ...
                        # 0C: routed through the single canonical write path (INV-4).
                        OuterMemoryWriter(send_queue, name).emit(OuterMemoryEvent(
        ...
        # confidence since the hypothesis was formed?
            ...
            try:
                ...
                if _haov_consumer == "knowledge":
                    ...
                    # 2026-05-17 — only log at INFO when we have a topic
                    # AND the verification produced a real signal. The empty-
                    # topic path is now prevented at the producer
                    # (cgn_worker.py HAOV pump). Belt-and-suspenders: if a
                    # producer somehow still emits without topic, downgrade
                    # to DEBUG so /tmp/titan_brain.log stays quiet. See
                    # BUG-CGN-KNOWLEDGE-HAOV-NO-TOPIC-FIELD-20260517.
                    if _topic:
                        logger.info(
                            "[HAOV] Knowledge verify: topic='%s' conf %.3f→%.3f "
                            "usage %d→%d confirmed=%s",
                            _topic[:40], _conf_b, _conf_a,
                            _usage_b, _usage_a, _confirmed)
                    else:
                        logger.debug(
                            "[HAOV] Knowledge verify skipped: no topic in "
                            "test_ctx (producer gap, rFP pending)")
...
def _extract_concept_associations(topic: str, summary: str = "") -> list[str]:
    """Bug C closure 2026-05-13 — populate knowledge_concepts.associations.

    Decomposes the topic (and a small slice of summary) into a deduped
    list of meaningful tokens for composed_thought pool A
    co-occurrence matching. Pool A walks recent rows and looks for any
    other concept whose own associations share a token with this one
    (modulo the anchor=topic check), so we need >=1 token per row.

    Filters: lowercased, alphanumerics + hyphens only, ≥3 chars, not
    a stopword, dedup while preserving first-seen order. Caps at 12
    tokens to keep the JSON small.
    """
    import re as _re
    seen: set[str] = set()
    out: list[str] = []
    blob = f"{topic} {summary or ''}"
    # Split on non-alphanumeric, keep hyphens within tokens.
    for raw in _re.split(r"[^A-Za-z0-9\-]+", blob):
        tok = raw.strip("-").lower()
        if len(tok) < 3 or tok in _KC_ASSOC_STOPWORDS:
            continue
        if tok in seen:
            continue
        # Skip pure-numeric tokens (years, ids etc.)
        if tok.isdigit():
            continue
        seen.add(tok)
        out.append(tok)
...
def _merge_associations(db_path: str, topic: str,
                        fresh: list[str]) -> str:
    """Return JSON-encoded union of existing-row associations + ``fresh``,
    deduped (first-seen order preserved), capped at 12 tokens. Used by
    the UPDATE branch of `_ground_concept` so re-encountering a known
    topic grows its association set instead of overwriting it.
    Tolerates legacy rows with NULL / invalid-JSON associations."""
    existing: list[str] = []
    try:
        conn = sqlite3.connect(db_path, timeout=2.0)
        row = conn.execute(
            "SELECT associations FROM knowledge_concepts WHERE topic = ?",
            (topic,)
        ).fetchone()
        conn.close()
        if row and row[0]:
            try:
                ...
                if isinstance(parsed, list):
                    existing = [str(x) for x in parsed if isinstance(x, str)]
            except Exception:
                existing = []
    except Exception:
        existing = []
    seen: set[str] = set()
    out: list[str] = []
    for tok in existing + list(fresh):
...
def _ground_concept(db_path: str, topic: str, summary: str,
    ...
    now = time.time()
    associations = _extract_concept_associations(topic, summary)
    concept = {
        "topic": topic,
        "summary": summary,
    ...
    try:
        ...
        if existing:
            ...
            # Bug C closure: merge fresh associations into the existing
            # JSON list (dedupe by string identity, cap at 12). Use
            # COALESCE so legacy rows with NULL associations bootstrap
            # to the freshly-computed list.
            ...
            client.write(
                ...
                (summary, summary, new_conf, quality,
                 concept["neuromod_at_acquisition"],
                 _merge_associations(db_path, topic, associations),
                 now, topic),
            ...
            client.write(
                ...
                "requesting_consumer, associations, "
                "neuromod_at_acquisition, created_at) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
