...
Bus protocol:
  ...
  TEACHER_SIGNALS   (language → cognitive_worker)  — MSL concept signals + vocab updates
...
# ── Track 2 D2 (v1.2.1) — WORD_PERTURBATION_HINT cache ──────────────────────
# Per rFP_phase_c_self_improvement_subsystem_migration §2.A.7 + SPEC §8.5
# D-SPEC-38. outer_interface_worker emits WORD_PERTURBATION_HINT with TTL
# ≤200ms; language_worker correlates with the corresponding SPEAK_REQUEST
# (same request_id) and merges the perturbations into the SPEAK payload.
#
# Cache shape: {request_id: (received_ts: float, hint_payload: dict)}.
# TTL enforced on lookup. Periodic cleanup in the message loop sweep drops
# entries older than TTL × 5 (1s default) to bound memory.
#
# Closes deferral D2 from the Phase A+B audit (chain was broken at the
# consumer end before this commit).
_word_perturbation_hints: dict = {}
_WORD_PERTURBATION_HINT_TTL_S = 0.2
_WORD_PERTURBATION_HINT_GC_THRESHOLD_S = 1.0


def _cache_word_perturbation_hint(payload: dict) -> None:
    """Cache a WORD_PERTURBATION_HINT for the SPEAK_REQUEST consumer to look up.

    Drop the hint if request_id is missing (defensive — outer_interface_worker
    always sets it per A6 _handle_speak_request_pending). Concurrent-write
    safety: this runs from the single language_worker dispatch loop, no
    cross-thread access — dict.set is atomic under the GIL.
    """
    import time as _time
    request_id = payload.get("request_id")
    if not isinstance(request_id, str) or not request_id:
        return
    _word_perturbation_hints[request_id] = (_time.time(), payload)


def _lookup_word_perturbation_hint(request_id: str) -> dict | None:
    """Look up a cached hint by request_id, enforcing the ≤200ms TTL.

    Returns the perturbations dict (str → float) if a fresh hint exists,
    None otherwise. Entry is popped on successful lookup (one-shot consumption).
    """
    import time as _time
    if not isinstance(request_id, str) or not request_id:
        return None
    entry = _word_perturbation_hints.pop(request_id, None)
    if entry is None:
        return None
    received_ts, payload = entry
    age = _time.time() - received_ts
    if age > _WORD_PERTURBATION_HINT_TTL_S:
        # Stale — discard (popped already, so no further action needed).
        return None
    return payload.get("perturbations") or {}


def _gc_word_perturbation_hints() -> None:
    """Drop hints older than _WORD_PERTURBATION_HINT_GC_THRESHOLD_S to bound
    memory. Called from the dispatch loop on each WORD_PERTURBATION_HINT
    receipt + every 100 iterations (lightweight)."""
    import time as _time
    now = _time.time()
    expired = [
        rid for rid, (ts, _) in _word_perturbation_hints.items()
        if now - ts > _WORD_PERTURBATION_HINT_GC_THRESHOLD_S
    ]
    for rid in expired:
        _word_perturbation_hints.pop(rid, None)
...
def language_worker_main(recv_queue, send_queue, name: str, config: dict) -> None:
    ...
    try:
        from titan_hcl.logic.grammar_patterns import GrammarPatternLibrary
    ...
    narrator = None
    try:
        from titan_hcl.logic.action_narrator import ActionNarrator
    ...
    try:
        from titan_hcl.logic.meta_cgn import EdgeDetector as _P7EdgeDet
    ...
    # 2026-05-17 — BUG-LANGUAGE-TEACHER-QUEUE-RESET-ON-RESTART. Queue was
    # in-memory only; on every worker restart it reset to []. The fire gate
    # at line ~597 requires non-empty queue → no fire → 46h teacher silence
    # observed fleet-wide. Persist the queue alongside the existing
    # compositions_since/last_fire_time so the time-based fallback can
    # actually fire after restart.
    #
    # Cap saved queue at 50 entries (most recent) to bound disk + load time.
    # If real queue grows beyond cap, in-memory keeps all entries — only
    # the persisted snapshot is truncated.
    _PERSISTED_QUEUE_MAX = 50
        ...
        try:
            if os.path.exists(teacher_state_path):
                with open(teacher_state_path) as f:
                    ...
                    # Migrate old format: queue field added 2026-05-17.
                    # Old state files won't have it — default to empty list.
    ...
    def _save_teacher_state(comp_since, last_fire, queue=None):
        try:
            tmp = teacher_state_path + ".tmp"
            payload = {"compositions_since": comp_since,
                       "last_fire_time": last_fire}
            # Optional queue snapshot — keep last N entries by insertion order.
            # Caller may omit (None) to skip queue update (preserves whatever
            # snapshot was last saved). Pass [] to explicitly clear.
            if queue is not None:
                payload["queue"] = list(queue)[-_PERSISTED_QUEUE_MAX:]
    ...
    teacher_queue = list(_teacher_state.get("queue", []) or [])
    ...
    if teacher_queue:
        logger.info(
            "[LanguageWorker] Teacher queue restored: %d entries",
            len(teacher_queue))
    ...
    try:
        ...
        from titan_hcl.logic.events_teacher_state_publisher import (
            EventsTeacherStatePublisher)
        from titan_hcl.logic.language_pipeline import update_language_stats
        from titan_hcl.logic.language_state_publisher import (
    ...
    last_periodic_run = 0.0  # 1Hz gate for periodic work block
    ...
    while True:
        ...
        try:
            ...
            pass  # no msg this cycle — periodic work below runs unconditionally
        except (KeyboardInterrupt, SystemExit):
            break

        # === Periodic work — runs every iteration, gated to 1Hz ===
        # Moved from inside `except Empty:` (2026-05-19) — periodic checks
        # were silently skipped fleet-wide because `recv_queue` was constantly
        # busy with EPOCH_TICK / SOCIAL_PERCEPTION / CGN_* / etc., so the 5s
        # timeout rarely fired and `except Empty:` rarely ran. Teacher trigger,
        # silence alert, bootstrap, conversation timeout, consolidation,
        # grounding, HAOV hypothesis, teacher pending-timeout, and
        # META_LANGUAGE_REWARD drain all lived in that branch and silently
        # never executed under normal bus load. The 1Hz outer gate prevents
        # over-execution; inner blocks have their own time gates (30s stats,
        # teacher_fallback_timeout, etc.) which now actually fire.
        _now_periodic = time.time()
        if _now_periodic - last_periodic_run >= 1.0:
            last_periodic_run = _now_periodic
            ...
            if now - last_stats_broadcast >= stats_broadcast_interval:
                ...
                # Debug trace (2026-05-19) — log trigger condition vars
                # alongside every stats broadcast. Removes locals()-based
                # counter that didn't fire as expected. Fires every 30s.
                _t_since = max(0.0, now - teacher_last_fire_time)
                _t_should = teacher_queue and teacher_pending_since == 0 and (
                    teacher_compositions_since >= teacher_interval
                    or _t_since >= teacher_fallback_timeout
                )
                logger.info(
                    "[LanguageWorker] trigger_eval queue=%d pending_since=%.0fs "
                    "comp=%d/%d since_teach=%.0fs fbk=%.0fs should=%s teacher=%s",
                    len(teacher_queue), teacher_pending_since,
                    teacher_compositions_since, teacher_interval,
                    _t_since, teacher_fallback_timeout,
                    bool(_t_should), persistent_teacher is not None,
                )
            ...
            if should_trigger:
                try:
                    ...
                    _save_teacher_state(teacher_compositions_since, teacher_last_fire_time, teacher_queue)
                    logger.info(
                        "[LanguageWorker] Teacher fired (reason=%s, queue=%d, comp_since=%d)",
                        _trigger_reason, len(teacher_queue), teacher_compositions_since)
                except Exception as e:
                    # 2026-05-19: was DEBUG (invisible) — promoted to WARNING
                    # so the silent-skip root cause surfaces in journalctl.
                    logger.warning("[LanguageWorker] Teacher trigger error: %s", e, exc_info=True)
            ...
            if not teacher_queue and teacher_pending_since == 0:
                from titan_hcl.logic.language_pipeline import should_bootstrap
                if should_bootstrap(
                    ...
                    try:
                        from titan_hcl.logic.language_teacher import BOOTSTRAP_MODE
            ...
            if (now - _grounding_last_check >= _grounding_interval
                ...
                try:
                    ...
                    for _gnd_w in _gnd_candidates:
                        ...
                        from titan_hcl.logic.language_teacher import META_FEEDBACK_MODE
                        ...
                        for _et_w in _et_candidates:
                            ...
                            from titan_hcl.logic.language_teacher import EMBODIED_TEACHING_MODE
        ...
        # If no message arrived, skip the message handler below
        if msg is None:
            continue
        ...
        # ── WORD_PERTURBATION_HINT (Track 2 D2 v1.2.1) ────────────────
        # outer_interface_worker emits per SPEAK_REQUEST_PENDING precursor.
        # Cache by request_id with ≤200ms TTL so the SPEAK_REQUEST handler
        # (next branch) can merge perturbations into the language pipeline.
        # Per rFP §2.A.7 + SPEC §8.5 D-SPEC-38.
        elif msg_type == bus.WORD_PERTURBATION_HINT:
            _cache_word_perturbation_hint(msg.get("payload", {}))
            _gc_word_perturbation_hints()
            continue
                                            ...
                                            for _src, _tgt in [(_cw1, _cw2), (_cw2, _cw1)]:
                                                ...
                                                if _row and _row[0]:
                                                    ...
                                                    if not _found and _mc:
                                                        ...
                                                        from titan_hcl.persistence import get_client as _cooc_gc
                                                ...
                                                if _row and _row[0]:
                                                    ...
                                                    if _mc:
                                                        ...
                                                        _mc[-1]["associations"].append(
                                                            [_tgt, _ml_assoc_type])
                                                        from titan_hcl.persistence import get_client as _ml_gc
        ...
        # ── CGN_WEIGHTS_MAJOR — weights updated (client auto-reloads from /dev/shm) ──
            ...
            if cgn:
                try:
                    ...
                    try:
                        from titan_hcl.logic.language_teacher import ARC_VOCABULARY_MAP
                        _dream_priority = []
        ...
        # associations. This creates emergent social-semantic connections.
            ...
            if cgn:
                try:
                    ...
                    # comprehending an external message is a language experience
                    # (mirrors deleted spirit_worker.py:8955 source="external").
                    # outcome = recognized fraction. Distinct coalesce sub-stream
                    # so it isn't starved by self-composition records.
                    try:
                        _sp_total = max(1, len(_sp_words_raw))
                        _sp_recog = len(_sp_vocab_words)
                        bus.emit_experience_record(
                            send_queue, name,
                            domain="language",
                            action_taken="comprehend:%d/%d" % (
                                _sp_recog, _sp_total),
                            outcome_score=min(1.0, _sp_recog / _sp_total),
                            context={
                                "recognized_count": _sp_recog,
                                "grounded": _sp_grounded,
                                "total_words": _sp_total,
                                "source": "external",
                                "author": str(_sp_author)[:40],
                                "message_text": str(_sp_summary)[:160],
                            },
                            coalesce_key="language:comprehend",
                        )
                    except Exception as _sp_exp_err:
                        logger.debug(
                            "[LanguageWorker] comprehension EXPERIENCE_RECORD "
                            "emit raised: %s", _sp_exp_err)
                                        ...
                                        if _row and _row[0]:
                                            ...
                                            if not _found and _mc:
                                                ...
                                                _mc[-1][
                                                    "associations"].append(
                                                    [_tgt, "SOCIAL_LINKED"])
                                                from titan_hcl.persistence import get_client as _sp_gc
...
def _check_teacher_trigger(
    ...
    from titan_hcl.logic.language_pipeline import build_teacher_request
...
def _handle_teacher_response(
    ...
    Sends TEACHER_SIGNALS to cognitive_worker (Phase D D-SPEC-116; was
    spirit_worker pre-retirement). cognitive restores the MSL concept-grounding
    + neuromod-nudge legs; other legs are superseded (perturbation→Rust) or
    covered elsewhere (recipes→vocab DB/narrator, conversation_eval→EXPERIENCE_RECORD).
    """
    ...
    if mode == "conversation_eval" and response:
        ...
        _send_msg(send_queue, bus.TEACHER_SIGNALS, name, "cognitive_worker", {
            "perturbation_deltas": [],
        ...
        try:
            ...
            if grounding.get("dimensions"):
                ...
                if row:
                    ...
                    conn.close()
                    from titan_hcl.persistence import get_client as _mf_gc
        ...
        try:
            ...
            if prescription.get("shifts"):
                ...
                if row:
                    ...
                    conn.close()
                    from titan_hcl.persistence import get_client as _et_gc
                ...
                if unknown_words:
                    ...
                    for acq_w in unknown_words[:acq_limit]:
                        ...
                        if not exists:
                            ...
                            from titan_hcl.persistence import get_client as _acq_gc
    ...
    # ── Send TEACHER_SIGNALS to cognitive_worker (Phase D D-SPEC-116) ──
    _send_msg(send_queue, bus.TEACHER_SIGNALS, name, "cognitive_worker", {
        "perturbation_deltas": perturbation_deltas,
        "msl_signals": msl_signals,
...
def _handle_speak_request(
    ...
    # Track 2 (v1.2.1) — word_perturbations from outer_interface_worker's
    # WORD_PERTURBATION_HINT (correlated by request_id with ≤200ms TTL).
    # Per SPEC §8.5 D-SPEC-38 schema extension + D2 closure (chain wire).
    #
    # Lookup precedence:
    #   1. payload["word_perturbations"] — if cognitive_worker pre-filled
    #      (legacy / direct path; today cognitive_worker doesn't fill this).
    #   2. _word_perturbation_hints cache by request_id — populated by the
    #      WORD_PERTURBATION_HINT dispatch branch immediately above
    #      _handle_speak_request in the main loop. One-shot consumption
    #      (entry popped on lookup); ≤200ms TTL enforced.
    #   3. Fallback empty — _handle_speak_request falls back to per-word
    #      narrator.get_word_perturbation() (pre-v1.2.1 behavior).
    request_id = payload.get("request_id")
    word_perturbations = payload.get("word_perturbations") or {}
    if not word_perturbations and isinstance(request_id, str):
        cached = _lookup_word_perturbation_hint(request_id)
        if cached:
            word_perturbations = cached
    if word_perturbations:
        logger.info(
            "[LanguageWorker] SPEAK_REQUEST consumed perturbations N=%d "
            "(request_id=%s, epoch=%d)",
            len(word_perturbations),
            (request_id[:8] if isinstance(request_id, str) else "n/a"),
            epoch_id)
    ...
    if eb_data:
        ...
        try:
            from titan_hcl.logic.experience_orchestrator import ExperienceBias
    ...
    if not result or not result.get("sentence"):
        result = compose_sentence(
            ...
            concept_confidences=concept_confidences,
            max_level=max_level,
            # Track 2 D3 (v1.2.1): pass word_perturbations from
            # WORD_PERTURBATION_HINT correlation. compose_sentence reorders
            # vocabulary by perturbation weight (descending) BEFORE calling
            # composition_engine.compose — high-perturbation words appear
            # earlier in the selector's candidate list, biasing selection
            # toward outer_interface_worker's narrator-recommended words.
            # When word_perturbations is empty/None, this is a no-op (full
            # backward-compatible behavior).
            word_perturbations=word_perturbations,
        )
    ...
    # (rFP_experience_distillation_phase_c) — self-composition is a language
    # experience (mirrors deleted spirit_worker.py:10920 direct_composition
    # record). Emit EXPERIENCE_RECORD; cognitive_worker enriches + persists.
    # Targeted/non-blocking/event-gated per bus-hygiene §3.1.
    try:
        bus.emit_experience_record(
            send_queue, name,
            domain="language",
            action_taken=sentence[:120],
            outcome_score=float(confidence),
            context={
                "level": level,
                "words_used": (result.get("words_used") or [])[:10],
                "template": str(result.get("template", ""))[:60],
                "source": "direct_composition",
            },
            epoch_id=int(epoch_id or 0),
            coalesce_key="language:compose",
        )
    except Exception as _exp_err:
        logger.debug(
            "[LanguageWorker] EXPERIENCE_RECORD emit raised: %s", _exp_err)
