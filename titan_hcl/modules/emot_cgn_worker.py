...
def emot_cgn_worker_main(recv_queue, send_queue, name: str,
    ...
    # ShmReaderBank for trajectory_state + cgn_beta_state SHM slots.
    # Replaces TRAJECTORY_UPDATE + CGN_BETA_SNAPSHOT bus subscriptions
    # per G18 — state transport via SHM, not bus. Read in the bundle
    # assembly path immediately before write (fresh values per bundle).
    try:
        from titan_hcl.api.shm_reader_bank import ShmReaderBank
        emot_shm_bank = ShmReaderBank(titan_id=titan_id)
        logger.info(
            "[EmotCGNWorker] ShmReaderBank attached (reads "
            "trajectory_state.bin + cgn_beta_state.bin per "
            "SPEC §7.1 D-SPEC-69)")
    except Exception as _bank_err:
        logger.warning(
            "[EmotCGNWorker] ShmReaderBank init failed: %s — "
            "trajectory + cgn_beta substrate caches will stay stale",
            _bank_err)
        emot_shm_bank = None
    ...
    # RFP_meta-reasoning_CGN_FIX.md Fix5 — boot-gate the routine pulse.
    # Cold-boot transient: first pulse can fire ~120s after worker start
    # but downstream meta_service handlers (cognitive/self_reflection/
    # memory) may still be in "starting" state for ~150s post-restart.
    # The early pulses then resolver_timeout at 5s; consumer sees failure.
    # Solution: gate the routine pulse on a boot warmup window so the
    # first pulse fires after all workers have settled.
    EMOT_META_PULSE_BOOT_GRACE_S = float(
        cfg.get("emot_meta_pulse_boot_grace_s", 180.0))
    _emot_meta_boot_ts = time.time()
    ...
    def _write_state_to_shm():
        ...
        try:
            ...
            # rFP_dead_dim_wiring_fix §2.E + §2.F / SPEC §7.1 + D-SPEC-69
            # v1.14.0: refresh trajectory + cgn_beta from SHM slots before
            # bundle write. G18-pure (state via SHM, not bus). Closes the
            # degenerate-value emit class from PART B §8 era (TRAJECTORY_UPDATE
            # used to send [0.5, 0.5] when state_vector dict-pipe stale;
            # CGN_BETA_SNAPSHOT used to send all-0.5 default).
            if emot_shm_bank is not None:
                try:
                    _traj_snap = emot_shm_bank.read_trajectory_state()
                    if _traj_snap:
                        worker_state["last_trajectory_2d"] = list(
                            _traj_snap.get("trajectory_2d") or [])
                except Exception as _e_t:
                    swallow_warn(
                        '[EmotCGNWorker] trajectory_state SHM read failed', _e_t,
                        key="modules.emot_cgn_worker.trajectory_state_read_error",
                        throttle=100)
                try:
                    _beta_snap = emot_shm_bank.read_cgn_beta_state()
                    if _beta_snap:
                        from titan_hcl.logic.emot_bundle_protocol import (
                            CGN_CONSUMERS as _CGN_C)
                        _vbc = _beta_snap.get("values_by_consumer") or {}
                        worker_state["last_cgn_beta_states_8d"] = [
                            float(_vbc.get(c, 0.5)) for c in _CGN_C
                        ]
                except Exception as _e_b:
                    swallow_warn(
                        '[EmotCGNWorker] cgn_beta_state SHM read failed', _e_b,
                        key="modules.emot_cgn_worker.cgn_beta_state_read_error",
                        throttle=100)
    ...
    while True:
        ...
        # session2-soak. Session 3 (RFP_meta-reasoning_CGN_FIX.md): real
        # chain execution + suggested_action from downstream handlers.
        # Boot-gate (Fix5): skip pulse during cold-boot warmup window
        # (EMOT_META_PULSE_BOOT_GRACE_S, default 180s) so the first pulse
        # fires after all downstream meta_service workers have settled.
        # Eliminates ~3 resolver_timeout failures per restart.
        _emot_boot_age = now - _emot_meta_boot_ts
        if (_emot_boot_age >= EMOT_META_PULSE_BOOT_GRACE_S
                and now - _last_emot_meta_pulse_ts >= EMOT_META_PULSE_INTERVAL_S):
        ...
        # TRAJECTORY_UPDATE bus handler RETIRED in rFP_dead_dim_wiring_fix
        # §2.E / SPEC §7.1 + D-SPEC-69 v1.14.0 — trajectory_2d now sourced
        # from `trajectory_state.bin` SHM slot (read in bundle-assembly
        # path above per G18). Producer at meta_reasoning.py:2930 also
        # retired in the same commit.


        # ── NS_URGENCIES_UPDATE (ns_worker → emot_cgn) ───────────────
        # RFP_meta-reasoning_CGN_FIX.md §8 Stage 1 — direct subscription
        # for `ns_urgencies_11d` substrate key. Replaces the route via
        # spirit_worker._attach_emot_producer_ctx (D8 retirement).
        # Producer-side emit fires on each KERNEL_EPOCH_TICK in
        # ns_worker. Payload: {"urgencies_by_program": {REFLEX: f, ...}}
        # — order normalized to emot_bundle_protocol.NS_PROGRAMS here.
        elif msg_type == bus.NS_URGENCIES_UPDATE:
            try:
                from titan_hcl.logic.emot_bundle_protocol import NS_PROGRAMS
                ...
                swallow_warn('[EmotCGNWorker] NS_URGENCIES_UPDATE error', _e,
                             key="modules.emot_cgn_worker.ns_urgencies_update_error", throttle=100)

        # ── SPACE_TOPOLOGY_UPDATE (cognitive_worker → emot_cgn) ──────
        # RFP_meta-reasoning_CGN_FIX.md §8 — direct subscription for
        # the 30D space_topology key. Closes DEAD-DIM `space_topology`.
        # Producer: cognitive_worker on each consciousness epoch tick
        # (sources outer_lower_topology_10D + inner_lower_topology_10D +
        # whole/unified_spirit_topology_10D from coordinator).
        elif msg_type == bus.SPACE_TOPOLOGY_UPDATE:
            try:
                ...
                swallow_warn('[EmotCGNWorker] SPACE_TOPOLOGY_UPDATE error', _e,
                             key="modules.emot_cgn_worker.space_topology_update_error", throttle=100)

        # ── NEUROMOD_LEVELS_UPDATE (neuromod_worker → emot_cgn) ──────
        # RFP_meta-reasoning_CGN_FIX.md §8 — direct subscription for the
        # 6D neuromod levels key. Closes DEAD-DIM `neuromod_state`.
        # Producer: neuromod_worker on each 2.5s coalesce tick. Order
        # locked to (DA, 5HT, NE, ACh, Endorphin, GABA) per existing
        # bundle convention (see also EMOT_CHAIN_EVIDENCE ctx fields).
        elif msg_type == bus.NEUROMOD_LEVELS_UPDATE:
            try:
                nm6 = payload.get("levels_6d") or []
                if len(nm6) >= 6:
                    worker_state["last_neuromod_6d"] = [
                        float(v) for v in nm6[:6]]
            except Exception as _e:
                swallow_warn('[EmotCGNWorker] NEUROMOD_LEVELS_UPDATE error', _e,
                             key="modules.emot_cgn_worker.neuromod_levels_update_error", throttle=100)

        # ── PI_PHASE_UPDATE (cognitive_worker → emot_cgn) ────────────
        # RFP_meta-reasoning_CGN_FIX.md §8 — direct subscription for
        # the 6D sphere-clock phase tuple. Closes DEAD-DIM `pi_phase`.
        # Producer: cognitive_worker on each consciousness epoch tick
        # (reads coordinator._sphere_clocks_snapshot, populated from
        # SHM via shm_bank.read_sphere_clocks() per Phase C G18).
        # Order: (inner_body, outer_body, inner_mind, outer_mind,
        # inner_spirit, outer_spirit) — Trinity × inner/outer symmetry.
        elif msg_type == bus.PI_PHASE_UPDATE:
            try:
                pp6 = payload.get("pi_phase_6d") or []
                if len(pp6) >= 6:
                    worker_state["last_pi_phase_6d"] = [
                        float(v) for v in pp6[:6]]
            except Exception as _e:
                swallow_warn('[EmotCGNWorker] PI_PHASE_UPDATE error', _e,
                             key="modules.emot_cgn_worker.pi_phase_update_error", throttle=100)

        # CGN_BETA_SNAPSHOT bus handler RETIRED in rFP_dead_dim_wiring_fix
        # §2.F / SPEC §7.1 + D-SPEC-69 v1.14.0 — cgn_beta_states now
        # sourced from `cgn_beta_state.bin` SHM slot (read in bundle-
        # assembly path above per G18). Producer at cgn_worker.py:675
        # also retired in the same commit.
