...
def timechain_worker_main(recv_queue, send_queue, name: str, config: dict) -> None:
    ...
    if v2_config.get("enabled", False):
        try:
            from titan_hcl.logic.timechain_v2 import TimeChainOrchestrator
    ...
    while True:
        ...
        # awaiting CLI/HTTP senders. Tracked I-003.
        # RFP_meta-reasoning_CGN_FIX.md Chunk B.7b Fix2: echo correlation_id
        # from request payload so meta_service resolver's pending-Future
        # cache (correlation_id → asyncio.Future) can resolve when this
        # response arrives at cognitive_worker (via the "meta_service" bus
        # alias). Without this echo the resolver awaits its 5s timeout.
            ...
            try:
                ...
                if _cid:
                    _resp["correlation_id"] = _cid
                _send_msg("TIMECHAIN_QUERY_RESP", src, _resp)
            except Exception as e:
                _err_resp = {"error": str(e)}
                if _cid:
                    _err_resp["correlation_id"] = _cid
                _send_msg("TIMECHAIN_QUERY_RESP", src, _err_resp)
        ...
        # Tracked as I-003 (intentionally not flagged as deaf-ear).
            ...
            # RFP_meta-reasoning_CGN_FIX.md Chunk B.7b Fix2 — same
            # correlation_id passthrough pattern as TIMECHAIN_QUERY above
            # (request payload → response payload). Applies to all 5
            # consumer API operations so meta_service timechain.{recall,
            # check,compare,aggregate,similar} resolvers complete without
            # hitting the 5s dispatch timeout.
            _v2_cid = payload.get("correlation_id")
            if not orchestrator:
                _err = {"error": "v2 not enabled"}
                if _v2_cid:
                    _err["correlation_id"] = _v2_cid
                _send_msg(msg_type + "_RESP", src, _err)
                continue
            try:
                ...
                # field in Session 2.
                    ...
                    _r = {"results": result}
                    if _v2_cid: _r["correlation_id"] = _v2_cid
                    _send_msg("TIMECHAIN_SIMILAR_RESP", src, _r)
            except Exception as e:
                logger.warning("[TimeChain] Consumer API error (%s): %s",
                               msg_type, e)
                _err_resp = {"error": str(e)}
                if _v2_cid: _err_resp["correlation_id"] = _v2_cid
                _send_msg(msg_type + "_RESP", src, _err_resp)
