...
class DependencyAction(str, enum.Enum):
    """SPEC §11.G.2.5 (D-SPEC-90, v1.29.0) — dependency activation mode.

    PROBE (default, backward-compatible) is the §11.G.2 respawn-only behavior:
    Guardian invokes `dep.check()` at pre-respawn time and blocks respawn if any
    critical dep is down.

    ENSURE_RUNNING extends the contract to first-start: when Guardian starts the
    dependent (autostart or lazy-wake), it first calls `_ensure_dep_running` for
    each `ENSURE_RUNNING + CRITICAL + MODULE` dep, recursively starting any
    STOPPED registered dep. Closes the lazy-start chicken-and-egg discovered
    post-§4.D meditation_worker extraction (memory was `autostart=False, lazy=True`
    and no subprocess could wake it from the parent's `MemoryProxy._ensure_started`
    bridge).
    """
...
class Dependency:
    ...
    Per SPEC §11.G.2.5 (D-SPEC-90, v1.29.0) — when `action=ENSURE_RUNNING`,
    Guardian additionally pre-starts the dep at the dependent's first-start
    if the dep is registered + STOPPED. PROBE (default) preserves legacy
    respawn-only semantics.

    `check` is a callable returning True iff the dependency is currently
    healthy. Examples (declared per-module in titan_hcl/core/plugin.py):
    ...
    action: DependencyAction = DependencyAction.PROBE
