...
def build_catalog(bus, guardian, config, *, titan_id: str, kernel=None) -> None:
    ...
    guardian.register(ModuleSpec(
        ...
        # (SynthesisVectorStore + EngineRecall) into synthesis_worker; the cap was
        # raised 350 → 700 to cover it. (Earlier revisions of this note sized it
        # against a ~317 MB **fastembed** model — OBSOLETE since the migration
        # below; the embedder is now llama.cpp, flat ~197 MB.)
        #
        ...
        # The 700 cap is sized against REAL RESIDENT (~459 MB → RssAnon ~498 MB
        # live), which is comfortable. As of 2026-06-16 the guardian ENFORCES
        # RssAnon, not VmRSS (orchestrator/core.py `_get_rss_mb`), so the
        # reclaimable FAISS/Kuzu/DuckDB mmap page-cache (~207 MB on T1's large
        # dataset) no longer inflates the enforced figure. The 2026-06-16 interim
        # 1500 VmRSS band-aid is REVERTED — 700 is correct against real memory;
        # a genuine RssAnon breach now THROTTLES, never respawns (INV-SUP-2).
        # (Earlier VmRSS false-OOM restart loop that prompted the band-aid: T1
        # mainnet 2026-06-15/16, verified RssAnon=498 vs VmRSS=706 via telemetry.)
        rss_limit_mb=700,
        autostart=True,
        lazy=False,
        heartbeat_timeout=60.0,
    ...
    _HORMONAL_WORKER_BROADCAST_TOPICS = _STATE_WORKER_BROADCAST_TOPICS + [
        ...
        # NOTE: HORMONE_CONSUME is intentionally NOT here — EXPRESSION
        # consumption is applied by cognitive_worker against the NNS
        # HormonalSystem (the nns_hormonal_state.bin instance expression reads),
        # not this worker's separate hormonal_state.bin (2026-06-01 correction).
    ]
