...
class OuterReasoningEngine:
    ...
    def __init__(self, config: Optional[dict] = None, save_dir: Optional[str] = None):
        ...
        # `gut_agreement` is part of the reasoning-engine interface the meta
        # DELEGATE/_check_delegate seam reads (meta_reasoning.py:_check_delegate
        # reads `reasoning_engine.gut_agreement`). The OUTER reasoner has NO gut
        # coupling (no NS-program urgencies) → it stays a neutral constant 0.5
        # ("no signal", honest; nothing fabricated). Kept as an attribute so the
        # shared meta DELEGATE seam treats the outer engine as a drop-in.
