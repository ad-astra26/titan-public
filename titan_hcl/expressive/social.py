...
class SocialManager:
    ...
    def __init__(self, metabolism_client, mood_engine=None, memory=None,
        ...
        # Dry-run mode: wired by TitanHCL when [endurance].social_dry_run = true
        self._dry_run = False
    ...
    async def _get_smart_session(self):
        ...
        # DISABLED: All X API calls must go through SocialXGateway (titan_hcl/logic/social_x_gateway.py)
    ...
    async def _generate_contextual_reply(
        ...
        try:
            ...
            if not _internal_key:
                raise ValueError("No internal_key for /v4/llm-distill")
            ...
            reply = await distill_via_http_async(
                ...
                max_tokens=100,
                temperature=0.8,
                consumer="social_manager_contextual_reply",
                timeout_s=30.0,
            )
            if not reply:
                raise ValueError("empty reply from /v4/llm-distill")
