...
class InnerMemoryWriterClient:
    ...
    def _loop_main(self) -> None:
        ...
        try:
            ...
            # BUG-T1-IMW-CLIENT-LOOP-CRASH-EVENT-LOOP-STOPPED-20260516
            #
            # close() at line 642 calls `self._loop.call_soon_threadsafe(
            # self._loop.stop)` from a non-loop thread (Guardian shutdown,
            # worker_swap_handler hot-reload). When that fires while
            # `_init_and_run` is mid-await (e.g. in `_replay_own_journal`
            # or the `while not self._closed: await asyncio.sleep(0.5)`
            # heartbeat), `run_until_complete` raises:
            #   RuntimeError("Event loop stopped before Future completed.")
            # That's the EXPECTED outcome of an externally-stopped loop —
            # not a crash. Pre-fix log was ERROR + full traceback, every
            # ~60s during the reconnect cycle, polluting brain logs.
            #
            # Distinguish: if `self._closed` is True (caller invoked close()),
            # this is clean shutdown — log INFO. Otherwise it's a real
            # crash and we keep the ERROR + traceback path.
            _msg = str(e)
            _shutdown_msg = "Event loop stopped before Future completed" in _msg
            if self._closed and _shutdown_msg:
                logger.info(
                    "[imw.client] loop exited via shutdown signal (expected)")
            else:
