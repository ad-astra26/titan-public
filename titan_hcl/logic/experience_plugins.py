...
from titan_hcl.logic.experience_orchestrator import ExperiencePlugin
...
# ── Phase 1 enrichment plugins (rFP_experience_distillation_phase_c §5) ──
# Extend distillation from the original 4 domains to the wisdom-bearing CGN
# consumer streams (Maker-ordered: knowledge + self_model + meta_reasoning first).

class KnowledgePlugin(ExperiencePlugin):
    """Knowledge grounding — concept grounding + impasse resolution.

    Perception key (20D):
      [0:5]   Inner Mind Thinking 5D    — reasoning state [state 5:10]
      [5:10]  Inner Spirit SAT 5D       — truth/grounding sub-layer [state 20:25]
      [10:15] Outer Mind 5D             — external concept perception [state 70:75]
      [15:20] Hormonal context 5D       — curiosity drive
    """
    ...
    def extract_perception_key(self, context: dict) -> list[float]:
        ...
        if len(state) >= 75:
            return (
                _safe_slice(state, 5, 10)       # Inner Mind Thinking
                + _safe_slice(state, 20, 25)    # Inner Spirit SAT (truth)
                + _safe_slice(state, 70, 75)    # Outer Mind
                + _get_hormones_5d(context)      # Hormonal context
            )
        return (
            _safe_slice(context.get("inner_mind", []), 0, 5)
            + _safe_slice(context.get("inner_spirit", []), 0, 5)
            + [0.5] * 5
            + _get_hormones_5d(context)
        )

    def compute_outcome_score(self, result: dict) -> float:
        if result.get("impasse_resolved"):
            return min(1.0, max(0.5, float(result.get("confidence", 0.6))))
        return min(1.0, max(0.0, float(
            result.get("confidence", result.get("grounding", 0.0)))))
    ...
    def summarize_for_distillation(self, experiences: list[dict]) -> dict:
        ...
        resolved = sum(
            ...
            if (e.get("context") or {}).get("impasse_resolved"))
        return {
            "pattern": (f"knowledge:avg={avg:.2f},resolved={resolved},"
                        f"n={len(experiences)}"),
            "avg_confidence": avg,
            "impasses_resolved": resolved,
        }


class SelfModelPlugin(ExperiencePlugin):
    """Self-model — reflection depth + coherence gain.

    Perception key (20D):
      [0:10]  Inner Spirit CHIT 10D     — self-awareness sub-layer [state 35:45]
      [10:15] Inner Mind Willing 5D     — agency-of-self [state 15:20]
      [15:20] Hormonal context 5D       — reflection drive
    """
    ...
    def extract_perception_key(self, context: dict) -> list[float]:
        ...
        if len(state) >= 45:
            return (
                _safe_slice(state, 35, 45)      # Inner Spirit CHIT (self-aware)
                + _safe_slice(state, 15, 20)    # Inner Mind Willing
                + _get_hormones_5d(context)
            )
        return (
            _safe_slice(context.get("inner_spirit", []), 15, 25)
            + _safe_slice(context.get("inner_mind", []), 10, 15)
            + _get_hormones_5d(context)
        )

    def compute_outcome_score(self, result: dict) -> float:
        return min(1.0, max(0.0, float(
            result.get("coherence", result.get("depth", result.get("score", 0.5))))))

    def summarize_for_distillation(self, experiences: list[dict]) -> dict:
        scores = [e.get("outcome_score", 0) for e in experiences]
        avg = sum(scores) / max(1, len(scores))
        return {
            "pattern": f"self_model:coherence_avg={avg:.2f},n={len(experiences)}",
            "avg_coherence": avg,
        }


class MetaReasoningPlugin(ExperiencePlugin):
    """Meta-reasoning — eureka / wisdom crystallization.

    Perception key (20D):
      [0:15]  Inner Mind 15D            — full Think/Feel/Will reasoning [state 5:20]
      [15:20] Hormonal context 5D       — inspiration drive
    """
    ...
    def extract_perception_key(self, context: dict) -> list[float]:
        state = context.get("inner_state", [])
        if len(state) >= 20:
            return _safe_slice(state, 5, 20) + _get_hormones_5d(context)
        return (
            _safe_slice(context.get("inner_mind", []), 0, 15)
            + _get_hormones_5d(context)
        )

    def compute_outcome_score(self, result: dict) -> float:
        return min(1.0, max(0.0, float(
            result.get("strength", result.get("eureka_strength",
                                              result.get("score", 0.5))))))

    def summarize_for_distillation(self, experiences: list[dict]) -> dict:
        scores = [e.get("outcome_score", 0) for e in experiences]
        avg = sum(scores) / max(1, len(scores))
        eurekas = sum(1 for s in scores if s > 0.7)
        return {
            "pattern": (f"meta_reasoning:avg={avg:.2f},strong={eurekas},"
                        f"n={len(experiences)}"),
            "avg_strength": avg,
            "strong_eurekas": eurekas,
        }
