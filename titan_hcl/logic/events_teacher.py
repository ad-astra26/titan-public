...
class DistilledEvent:
    ...
    # B4 (INV-XENG-4, 2026-06-03) — author identity captured at search time so
    # the organic auto-follow policy can follow a discovered (non-followed)
    # voice without re-resolving its user_id. Empty for sources that don't
    # carry it (e.g. follower_timeline authors are already in the registry).
    author_id: str = ""
...
class EventsTeacherDB:
    ...
    def _init_db(self):
        ...
        try:
            ...
            pass
        # B4 (2026-06-03): author_id + follower count on felt_experiences so the
        # organic auto-follow policy can follow a discovered voice (captured at
        # search time — twitterapi.io returns it; we used to discard it).
        for _col, _ddl in (
            ("author_id", "ADD COLUMN author_id TEXT DEFAULT ''"),
            ("author_followers", "ADD COLUMN author_followers INTEGER DEFAULT 0"),
        ):
            try:
                conn.execute(f"ALTER TABLE felt_experiences {_ddl}")
            except sqlite3.OperationalError:
                pass
        conn.commit()
        conn.close()
    ...
    def store_felt_experience(self, titan_id: str, event: DistilledEvent,
        ...
        self._route_write(
            "INSERT INTO felt_experiences "
                ...
                "mode, window_id, created_at, author_id, author_followers) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (titan_id, event.source, event.author, event.topic,
                 ...
                 getattr(event, "author_id", "") or "",
                 int(getattr(event, "author_followers", 0) or 0)),
    ...
    def _fetch_topic_search(self, query: str, config: dict,
        ...
        try:
            ...
            for tw in tweets:
                ...
                # B4: capture the author's user_id + follower count (twitterapi.io
                # returns them on every search result) so the auto-follow policy
                # can follow a discovered non-followed voice without re-resolving.
                author_id = str(author.get("id", "") or "")
                author_followers = int(
                    author.get("followers",
                               author.get("followersCount",
                                          author.get("followers_count", 0))) or 0)
        ...
        def _distill_chunk(chunk: list[dict]) -> list[DistilledEvent]:
            ...
            for i, p in enumerate(parsed):
                ...
                out.append(DistilledEvent(
                    ...
                    timestamp=time.time(),
                    author_id=chunk[i].get("author_id", ""),
                    author_followers=int(chunk[i].get("author_followers", 0) or 0),
                ))
            return out
