...
D-SPEC-78 (SPEC v1.20.0): pure data classes EXTRACTED to `cgn_types.py`
to break the implicit torch dependency for non-torch callers (arc/session.py
grounding, haov_causal_generator type hints). New callers needing just the
data structures should import from `cgn_types` directly; importing from
`cgn` here pulls torch (libtorch + libtriton, ~100MB mmap cost). Only
`cgn_worker` + `sage` should hit this module.

See: titan-docs/rFP_concept_grounding_network.md
     titan-docs/rFP_chat_streaming_safety_first_ovg_async.md §9.1 Chunk α
"""
...
from titan_hcl.logic.haov_causal_generator import CausalGeneratorRegistry

# D-SPEC-78: re-export pure data classes from cgn_types for back-compat.
# Direct callers should import from cgn_types to avoid torch load.
from titan_hcl.logic.cgn_types import (  # noqa: F401  (re-export)
    ConceptFeatures,
    SensoryContext,
    GroundingAction,
    CGNConsumerConfig,
...
class ConceptGroundingNetwork:
    ...
    def get_stats(self) -> dict:
        ...
        if self._total_groundings > 0:
            avg_reward = self._total_rewards / self._total_groundings
            # SPEC §23.8 — CGN rewards are nominally [-1, 1]; map to [0, 1].
            avg_reward_norm = min(1.0, max(0.0, (avg_reward + 1.0) / 2.0))
        else:
            # No groundings yet — SPEC no-data signal is 0.5.
            avg_reward = 0.0
            avg_reward_norm = 0.5
        return {
            ...
            "avg_reward": round(avg_reward, 4),
            # SPEC §23.8 / rFP_phase_c_130d_rust_l1_port §3.1 — normalized
            # to [0,1] for outer_mind thinking[0] research_effectiveness +
            # outer_spirit CHIT[5] + ANANDA[9]. 0.5 = no-data sentinel.
            "avg_reward_norm": round(avg_reward_norm, 4),
            "buffer_size": self._buffer.size(),
