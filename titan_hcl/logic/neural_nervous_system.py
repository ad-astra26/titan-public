"""
titan_hcl/logic/neural_nervous_system.py — V5 Neural NervousSystem.
...
class NeuralNervousSystem:
    ...
    def _record_transition(self, name: str, observation: np.ndarray,
        ...
        # Auto-train every N transitions — SUSPENDED during dreams. The
        # off-tick dream-consolidation suite (consolidate_training) trains the
        # SAME program nets; running online _train_all concurrently would race
        # the backprop / net.lr. No waking learning during sleep — perception
        # (forward/hormonal/signals above) still runs. Requires set_dreaming()
        # to be wired by the epoch driver (Phase C: cognitive_worker).
        # rFP_dream_consolidation_suite_offtick_restoration §8 / D-SPEC-105.
        if (not self._is_dreaming
                and self._total_transitions % self._train_every_n == 0):
            self._train_all()
