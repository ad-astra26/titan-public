...
KNOWN_QUESTION_TYPES = frozenset({
    ...
    "recall_context",
    "evaluate_option",
    "evaluate_trajectory",  # added 2026-05-14 — was used by emot_cgn_worker.py:521
                            # since 58f4936b (EmotMeta routine pulse) but never
                            # whitelisted on this end; routine pulse dormant for
                            # 14 days because of the validator rejection.
                            # Semantic: evaluate the trajectory of a cluster /
                            # state over time (vs evaluate_option which is for
...
FAILURE_MODES = frozenset({
    ...
    "recursion_cap",
    "impasse_hit",
    "not_yet_implemented",      # Session 1 dry-run handler (kept for fallback)
    # Session 3 (RFP_meta-reasoning_CGN_FIX.md PART A) — live-dispatch failures
    "resolver_unavailable",     # no resolver registered for the dispatched category
    "resolver_timeout",         # dispatch published but response did not arrive in time
    "resolver_error",           # resolver raised an exception during dispatch
})
...
def send_meta_request(
    ...
    msg = {
        ...
        # from spirit_worker (D8 retirement target) to cognitive_worker
        # (canonical home for MetaReasoningEngine + MetaCGNConsumer per
        # SPEC §9.B §4.A SHIPPED 2026-05-05). All 8 consumers route here
        # via this single helper, so the migration is one-line + lands
        # atomically.
...
def send_meta_outcome(
    ...
    msg = {
        ...
        "src": src or consumer_id,
        # RFP_meta-reasoning_CGN_FIX.md Chunk B.7b — MetaService relocated
        # to cognitive_worker; OUTCOMEs route there too.
        "dst": "cognitive_worker",
        "ts": time.time(),
        "rid": None,
        "payload": payload,
