"""
titan_hcl/logic/coding_explorer.py — Self-Directed Coding Explorer.
...
class CodingExplorer:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            # Track 2 D5 (v1.2.1) — observation counter from
            # self_reflection_worker B5 dispatcher.
            "noted_chains": getattr(self, "_noted_chains", 0),
            "last_noted_chain_ts": getattr(self, "_last_noted_chain_ts", 0.0),
        }

    # ── Track 2 D5 (v1.2.1) — chain observation surface ──────────────────
    #
    # self_reflection_worker's _handle_reasoning_stats (chunk B5) caches the
    # reasoning stats payload + calls this method so CodingExplorer can
    # accumulate chain-commit signal for the cross-pollination trigger
    # surface (per rFP_coding_explorer_activation.md §4.1). Until the
    # granular REASONING_COMMIT bus event ships, this is a counter-only
    # surface — chain body content isn't available via the *_STATS_UPDATED
    # proxy. Per Prime Directive #1 closure: the method exists + is called
    # + surfaces in get_stats; deeper chain-content integration waits on
    # the follow-up rFP that ships REASONING_COMMIT with chain body.

    def note_chain(self, reasoning_stats: dict) -> None:
        """Note a reasoning-chain event from cognitive_worker (via the
        REASONING_STATS_UPDATED bus event proxy).

        Bumps `_noted_chains` counter + caches latest stats + ts. Future
        REASONING_COMMIT extension (with chain body) will drive substantive
        cross-pollination logic; today this is the observation surface that
        closes the rFP §2.B.3 contract.
        """
        self._noted_chains = getattr(self, "_noted_chains", 0) + 1
        self._last_noted_chain_ts = time.time()
