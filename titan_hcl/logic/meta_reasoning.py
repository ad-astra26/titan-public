"""
titan_hcl/logic/meta_reasoning.py — Meta-reasoning: thinking about thinking.
...
STEP_REWARDS = {
    ...
    # rFP_titan_meta_outer_layer sub-modes — Bridge 1 (RECALL.topic),
    # Bridge 2 (EVALUATE.peer_cgn), §3 (DELEGATE.gap_fill). Values mirror
    # each primitive's existing average so these aren't preferred or
    # penalized vs. current modes until emergent rewards ramp in
    # (RFP_meta-reasoning_CGN_FIX.md §4.4 α-ramp).
    "RECALL.topic": 0.02,
    "EVALUATE.peer_cgn": 0.02,
    "DELEGATE.gap_fill": 0.0,
}
...
class MetaReasoningEngine(PrimitiveHandlersMixin):
    ...
    def __init__(self, config: dict = None, send_queue=None):
        ...
        # construct site in cognitive_worker._init_cognitive_engines (via
        # _merge_per_titan_dna). NOTE: meta-reasoning moved spirit→cognitive;
        # the merge moved with it (2026-05-20 regression fix — the cognitive
        # construct site had been passing the bare [meta_reasoning] section,
        # leaving this dict empty fleet-wide).
        ...
        # EmotCGNConsumer now lives in `titan_hcl/modules/emot_cgn_worker.py`
        ...
        # Lower reasoning-layer lifetime commit telemetry, cached each tick (the
        # counter lives on reasoning_engine, not here) so get_stats() can expose
        # it for self_reasoning Layer A novelty-gap detection.
        self._last_reasoning_commits = 0
        self._last_reasoning_chains = 0
        ...
        # Self-reasoning engine bridge (D-SPEC-70 v1.15.0 — closes F-8 fleet-wide).
        # Pre-Phase-C: meta_reasoning.set_self_reasoning(engine) was called
        # by spirit_worker at boot; introspect ran in-process. Post-D8-3
        # (commit 72f95a6b 2026-05-16): SelfReasoningEngine lives in
        # self_reflection_worker (separate process). _prim_introspect now
        # reads the latest result from inner_self_insight.bin SHM slot
        # (pre-warmed cache per G20) and fires a fire-and-forget
        # META_INTROSPECT_REQUEST bus event for the next tick's result.
        # _inner_self_insight_reader is attached by cognitive_worker post
        # construction (passes ShmReaderBank instance from state_refs).
        # _last_shm_insight_epoch tracks the last SHM payload epoch consumed
        # so Producer #13/#14 queues are populated once per insight, not
        # per read.
        self._inner_self_insight_reader = None
        self._last_shm_insight_epoch: int = -1
    ...
    def tick(self, state_132d, neuromods, reasoning_engine,
        ...
        if reasoning_engine is not None:
            self._last_reasoning_commits = int(
                getattr(reasoning_engine, "_total_commits", 0) or 0)
            self._last_reasoning_chains = int(
                getattr(reasoning_engine, "_total_chains", 0) or 0)
        ...
        if (not _g_sel_active
            ...
            try:
                from titan_hcl.logic.task_embedding import template_to_primitive_list
                ...
                swallow_warn('[logic.meta_reasoning] MetaReasoningEngine.tick: from titan_hcl.logic.task_embedding import template_to...', _tb_err,
        ...
        # INTROSPECT gate: maturity-gated + per-chain de-dup (max 1 per chain).
        # Pre-D-SPEC-70: also gated on `_sr.can_introspect` in-process cooldown
        # check (retired — engine now lives in self_reflection_worker; cooldown
        # is enforced inside that worker, request is fire-and-forget here).
    ...
    def save_all(self):
        ...
        if self._meta_cgn is not None:
            try:
                ...
                logger.warning("[META] META-CGN save failed: %s",
                               _mcgn_save_err)
        # Phase B (RFP_cgn_enhancements §9.2): persist the cross-consumer
        # composition store (the Level-B abstraction output) so accumulated
        # compositions survive restart (SPEC §line277 — statefulness mandates
        # periodic + graceful-shutdown persistence). The store loads on boot in
        # its __init__; this is the periodic save half of the contract.
        if self._composition_store is not None:
            try:
                self._composition_store.save()
            except Exception as _ccc_err:
                logger.warning("[META] CrossConsumerComposition save failed: %s",
                               _ccc_err)
        ...
        try:
            stats_payload = {
                ...
                # Phase B (§9.2) — persist the Level-B accumulator so matured
                # concepts already counted toward the trigger aren't lost on
                # restart (matured_emitted dedups emission, so they would never
                # re-arrive — they MUST persist here or the Level-B progress
                # "starts over" every restart).
                "concept_grounded_since_levelb": _safe_int(
                    "_concept_grounded_since_levelb"),
                "pending_concept_grounded": list(
                    getattr(self, "_pending_concept_grounded", []))[:50],
    ...
    def get_stats(self) -> dict:
        ...
        stats = {
            ...
            # Lower reasoning-layer commit telemetry (the counter lives on
            # reasoning_engine, the layer meta delegates to), cached each tick
            # — see tick(). self_reasoning Layer A reads this for novelty-gap
            # detection (rFP coding_explorer §4.1). The 2026-04-21 hotfix read
            # getattr(self, …) → always 0; properly wired 2026-06-11.
            "total_commits": self._last_reasoning_commits,
            "commit_rate": round(
                self._last_reasoning_commits
                / max(1, self._last_reasoning_chains), 4),
    ...
    def get_audit_stats(self) -> dict:
        ...
        return {
            ...
            "meta_cgn": (
                self._meta_cgn.get_stats() if self._meta_cgn is not None else {}
            ),
            "failsafe": (
                self._meta_cgn.get_failsafe_status()
                if self._meta_cgn is not None else {}
            ),
        }
    ...
    # set_self_reasoning() RETIRED in D-SPEC-70 v1.15.0 (2026-05-17).
    # Pre-Phase-C this setter was called by spirit_worker at boot to
    # attach a SelfReasoningEngine instance for in-process introspect
    # dispatch. Under Phase C the engine lives in self_reflection_worker
    # (separate process) and introspect dispatch flows via the
    # META_INTROSPECT_REQUEST bus event + inner_self_insight.bin SHM
    # slot bridge per §9.B + G18-G22. _prim_introspect reads the SHM
    # pre-warmed cache. Closes F-8 fleet-wide.
    ...
    def _start_chain(self, reason, state_132d):
        ...
        try:
            from titan_hcl.bus import emit_felt_cluster_update
            ...
            swallow_warn('[logic.meta_reasoning] MetaReasoningEngine._start_chain: from titan_hcl.bus import emit_felt_cluster_update', _swallow_exc,
        ...
        if self._chain_iql and self._chain_iql.enabled:
            try:
                from titan_hcl.logic.task_embedding import encode_task
        ...
        try:
            ...
            if not _live_keys:
                logger.info(
                    ...
                    "by design (graceful α<1.0 fallback per "
                    "RFP_meta-reasoning_CGN_FIX.md §4.5; static-reward "
                    "path retired when α reaches 1.0)", _total,
                )
            else:
                _summary = " ".join(
    ...
    def _conclude_chain(self, state_132d, chain_archive, meta_wisdom, autoencoder):
        ...
        try:
            ...
            try:
                ...
                swallow_warn('[logic.meta_reasoning] MetaReasoningEngine._conclude_chain: from titan_hcl.logic.emot_shm_protocol import ShmEmotR...', _swallow_exc,
            ...
            _emot_ctx = {
                ...
                "GABA": _nmval("GABA"),
                # Substrate fields (trajectory_2d, space_topology_30d,
                # ns_urgencies_11d, pi_phase_6d, neuromod_state_6d) are no
                # longer carried in EMOT_CHAIN_EVIDENCE ctx as of
                # RFP_meta-reasoning_CGN_FIX.md §8 (SPEC v1.9.6 D-SPEC-65,
                # 2026-05-16). emot_cgn_worker now caches them via
                # dedicated direct subscriptions to TRAJECTORY_UPDATE +
                # SPACE_TOPOLOGY_UPDATE + NS_URGENCIES_UPDATE +
                # PI_PHASE_UPDATE + NEUROMOD_LEVELS_UPDATE, retiring the
                # spirit_worker._attach_emot_producer_ctx bridge (D8).
                # The EMOT_CHAIN_EVIDENCE handler's `if "X" in ctx:`
                # checks gracefully skip these absent fields and preserve
                # the worker_state cache populated by direct subs.
                "monoculture_share": mcgn_ctx.get("monoculture_share", 0.0),
            ...
            from titan_hcl.bus import emit_emot_chain_evidence
            if self._send_queue is not None:
                emit_emot_chain_evidence(
                    ...
                    ctx=_emot_ctx)
                # TRAJECTORY_UPDATE bus emit RETIRED in rFP_dead_dim_wiring_fix
                # §2.E / SPEC §7.1 + D-SPEC-69 v1.14.0 — trajectory_2d now
                # flows via `trajectory_state.bin` SHM slot, written by
                # cognitive_worker (G21 single-writer) per consciousness
                # epoch directly from coordinator's freshly-computed
                # curvature + density (bypassing the broken state_vector
                # snapshot pipe that caused [0.5, 0.5] degenerate emit).
    ...
    def _load(self):
        ...
        if os.path.exists(stats_path):
            ...
            try:
                ...
                # Phase B (§9.2) — restore the Level-B accumulator (don't start over).
                self._concept_grounded_since_levelb = int(
                    s.get("concept_grounded_since_levelb", 0) or 0)
                _pcg = s.get("pending_concept_grounded", [])
                if isinstance(_pcg, list):
                    self._pending_concept_grounded = [
                        p for p in _pcg if isinstance(p, dict) and p.get("concept_id")][-50:]
