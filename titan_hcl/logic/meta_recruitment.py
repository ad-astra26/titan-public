...
# Resolver callables MAY be sync or async (RFP_meta-reasoning_CGN_FIX.md §4.1).
# Async is preferred per SPEC Preamble G19+G22 + §8.0.ter D-SPEC-48; sync allowed
# for exceptions documented in the resolver's docstring. The MetaService dispatcher
# inspects the returned object and awaits if it is a coroutine (Chunk B work).
ResolverReturn = Optional[Dict[str, Any]]
ResolverCallable = Callable[[str, Dict[str, Any]],
                            Union[ResolverReturn, Awaitable[ResolverReturn]]]
...
RECRUITMENT_CATALOG: Dict[str, List[str]] = {
    ...
    # RECALL.topic — rFP_titan_meta_outer_layer Bridge 1: topic-scoped
    # composed recall (concept via knowledge_worker + felt_history + inner
    # narrative). Recruiters mirror RECALL.entity + .experience scope.
    "RECALL.topic": [
        "semantic_graph.neighbors",
        "episodic_memory.search",
        "meta_wisdom.query_by_embedding",
    ...
    # EVALUATE.peer_cgn — rFP_titan_meta_outer_layer Bridge 2: cross-consumer
    # β-posterior comparison via outer_reader.peer_cgn_beta(consumer, topic).
    # Recruiters: self_reasoning.predict (compare own belief to peer mean) +
    # reasoning.consistency_check (flag divergence) + REFLECTION (catch dust).
    "EVALUATE.peer_cgn": [
        "self_reasoning.predict",
        "reasoning.consistency_check",
        "REFLECTION",
    ...
    # DELEGATE.gap_fill — rFP_titan_meta_outer_layer §3: pull fresh external
    # data when chain is thin (knowledge_search / X timeline / events_window).
    # NOT a recursive meta-chain; uses outer_reader gateway. Recruiters mirror
    # RECALL.entity scope (semantic_graph for known refs) plus chain_archive
    # for prior gap-fill matches that succeeded.
    "DELEGATE.gap_fill": [
        "semantic_graph.neighbors",
        "chain_archive.query",
        "pattern_primitives.match",
...
class MetaRecruitment:
    ...
    def register_resolver(self, category: str,
        ...
        at minimum {"success": bool, "output": any}, OR an Awaitable that
        resolves to the same.

        Async resolvers are preferred per SPEC Preamble G19+G22 + §8.0.ter
        D-SPEC-48 (publish-non-blocking) and per
        `RFP_meta-reasoning_CGN_FIX.md` §4.1. Sync resolvers are allowed only
        as an explicit exception documented in the resolver's docstring.

        The MetaService dispatcher (Chunk B / §4.1 schematic) inspects the
        returned value via `inspect.iscoroutine` and awaits if needed.

        Session 1 never calls this; Session 2 wires the 10 shells; Session 3
        (RFP_meta-reasoning_CGN_FIX.md PART A) replaces them with async
        live-dispatch resolvers.
        """
        if not callable(resolver_fn):
            raise ValueError(
                f"MetaRecruitment: resolver_fn for category={category!r} "
                f"must be callable")
        is_coro_fn = inspect.iscoroutinefunction(resolver_fn)
        self._resolvers[category] = resolver_fn
        logger.info(
            "[MetaRecruit] resolver registered for category='%s' (%s)",
            category, "async" if is_coro_fn else "sync",
        )
