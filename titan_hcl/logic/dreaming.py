"""
titan_hcl/logic/dreaming.py — Dreaming Cycle Engine (T6).
...
class DreamingEngine:
    ...
    def __init__(
        ...
        # v1.8.2 (D-SPEC-56): external wake-request flag, set by `request_wake()`
        # in response to DREAM_WAKE_FORWARD bus event from dream_state_worker
        # (chat-API maker-fast-wake + world-observer-interrupt). Checked in
        # `check_transition` DREAMING branch after min_dream_s gate — returns
        # END_DREAMING immediately, bypassing the consolidation-gated wake
        # conditions. Reason captured in `_wake_request_reason` for logging.
        self._wake_requested: bool = False
        self._wake_request_reason: str = ""
        # Force-dream wiring (2026-05-15 evening, post-§4.I D8-3 cleanup):
        # external dream-request flag, set by `request_dream()` in response
        # to FORCE_DREAM_REQUEST bus event (admin / maker testing /
        # maintenance / inspection path). Closes the orphan FORCE_DREAM_REQUEST
        # in `command_sender.py:120` whose handler lived in the deleted
        # spirit_worker dream-emit block (chunk I8 cleanup). Checked in
        # `check_transition` AWAKE branch right before the FORCE SLEEP path —
        # bypasses wake_inertia (matching FORCE SLEEP semantics).
        self._dream_requested: bool = False
        self._dream_request_reason: str = ""

        # Wake inertia: timestamp of last wake-up (sleep refractory period)
        self._last_wake_ts: float = 0.0

    # ── External wake-request API (v1.8.2 / D-SPEC-56) ──────────

    def request_wake(self, reason: str = "") -> None:
        """Request immediate end-of-dream from an external source.

        Called by cognitive_worker on DREAM_WAKE_FORWARD bus event receipt.
        Sets `_wake_requested` flag; the next `check_transition` invocation
        in the DREAMING branch (after `min_dream_s` gate) will return
        "END_DREAMING" and the flag self-clears. Idempotent — repeated calls
        before the next tick collapse to one wake.
        """
        if not self._wake_requested:
            logger.info(
                "[Dreaming] external wake request received (reason=%s)",
                reason or "unspecified")
        self._wake_requested = True
        self._wake_request_reason = reason or self._wake_request_reason

    # ── External dream-request API (force-dream, admin / test) ───

    def request_dream(self, reason: str = "") -> None:
        """Request immediate start-of-dream from an external source.

        Called by cognitive_worker on FORCE_DREAM_REQUEST bus event receipt
        (CommandSender admin path — maker testing / maintenance / inspection).
        Symmetric to `request_wake`. Sets `_dream_requested` flag; the next
        `check_transition` invocation in the AWAKE branch (right before the
        FORCE SLEEP path) will return "BEGIN_DREAMING" and the flag self-
        clears. Idempotent — repeated calls before the next tick collapse to
        one dream.
        """
        if not self._dream_requested:
            logger.info(
                "[Dreaming] external dream request received (reason=%s)",
                reason or "unspecified")
        self._dream_requested = True
        self._dream_request_reason = reason or self._dream_request_reason
    ...
    def check_transition(
        ...
        if not inner_state.is_dreaming:
            ...
            # Force-dream wiring (2026-05-15 evening, post-§4.I D8-3 cleanup):
            # external request via FORCE_DREAM_REQUEST → cognitive_worker →
            # request_dream(reason) → flag set here. Takes precedence over
            # consolidation/wake-inertia gates AND ahead of the FORCE SLEEP
            # safety path (which is metabolic-drain-driven). Used by maker
            # admin path for testing / maintenance / inspection.
            if self._dream_requested:
                self._onset_drain = metabolic_drain
                self._onset_undistilled = int(undistilled)
                logger.info(
                    "[Dreaming] external dream request honored (reason=%s) — "
                    "BEGIN_DREAMING (bypasses wake_inertia + drive gate)",
                    self._dream_request_reason or "unspecified")
                self._dream_requested = False
                self._dream_request_reason = ""
                return "BEGIN_DREAMING"
            ...
            # v1.8.2 (D-SPEC-56): external wake-request takes precedence over
            # consolidation gates (after min_dream_s respected). Used by chat-API
            # maker-fast-wake (DREAM_WAKE_REQUEST → dream_state_worker →
            # DREAM_WAKE_FORWARD → cognitive_worker → request_wake) + world-
            # observer DI:URGENT interrupts.
            if self._wake_requested:
                logger.info(
                    "[Dreaming] external wake request honored (reason=%s, "
                    "duration=%.0fs) — END_DREAMING",
                    self._wake_request_reason or "unspecified", dream_duration)
                self._wake_requested = False
                self._wake_request_reason = ""
                return "END_DREAMING"
