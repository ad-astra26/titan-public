...
LAYERING (read first): this module is the STORAGE SUBSTRATE (low layer) and is
NOT legacy. `timechain_v2.py` (TimeChainOrchestrator) is an ORCHESTRATION /
COGNITIVE ENGINE layered ON TOP — it wraps an instance of this class
(`TimeChainOrchestrator(timechain=TimeChain(...))`) and delegates every write to
this module's `commit_block()`. The "v2" suffix names a LAYER, not a successor:
both are live. This module is also used directly by meditation, timechain_integrity,
timechain_backup, output_verifier, dashboard, spirit_loop. Retire neither.
...
class TimeChain:
    ...
    # Phase 14 §3K.3.C — legacy-conversation provenance anchor.
    _CONVERSATION_ANCHOR_SOURCE = "phase14_conversation_anchor"
    _CONVERSATION_CHAT_MARKERS = ("verified_output", "chat")

    def write_conversation_legacy_anchor(self) -> Optional["Block"]:
        """Phase 14 §3K.3.C — additive cross-ref provenance anchor (idempotent).

        On a chain where the `conversation` primary was reseeded to a RELOCATED
        id (T2/T3 — fork 5 was held by a genesis-era sidechain, INV-Syn-26), the
        genuine historical chat blocks (tags ⊇ verified_output|chat) physically
        live in the legacy fork 5. This appends a SINGLE provenance-anchor block
        to the new conversation fork recording the legacy chat set (block hashes
        + merkle root + count + height range) + cross-refs into fork 5.

        ADDITIVE + IDEMPOTENT + sovereign-safe: never deletes/moves data
        (`directive_memory_preservation`); skips if an anchor already exists;
        a verified no-op on T1 (conversation == canonical id 5 — no relocation)
        and on fresh chains (no legacy occupant). Recall still reads the new
        fork; the anchor is provenance — a future reader can prove + locate the
        pre-Phase-14 chat history. Single-writer: only the timechain_worker
        (sole TimeChain owner) calls this, once, at boot after genesis.

        Returns the committed anchor Block, or None when there is nothing to do.
        """
        conv_id = self.resolve_fork_id("conversation")
        if conv_id is None or conv_id == FORK_CONVERSATION:
            return None  # absent, or canonical id 5 (T1-correct) — no relocation
        conn = sqlite3.connect(str(self._index_db_path))
        try:
            # Idempotency — already anchored?
            done = conn.execute(
                "SELECT 1 FROM block_index WHERE fork_id=? AND source=? LIMIT 1",
                (conv_id, self._CONVERSATION_ANCHOR_SOURCE)).fetchone()
            if done:
                return None
            # Genuine historical chat in the legacy slot (FORK_CONVERSATION=5).
            rows = conn.execute(
                "SELECT block_hash, block_height, tags FROM block_index "
                "WHERE fork_id=? ORDER BY block_height", (FORK_CONVERSATION,)
            ).fetchall()
        finally:
            conn.close()
        chat = [(bytes(bh), int(h)) for (bh, h, tags) in rows
                if any(m in (tags or "") for m in self._CONVERSATION_CHAT_MARKERS)]
        if not chat:
            return None  # nothing genuinely-conversation in the legacy fork
        hashes = [bh for bh, _ in chat]
        heights = [h for _, h in chat]
        # Binary merkle fold over the legacy chat block hashes.
        layer = list(hashes)
        while len(layer) > 1:
            layer = [sha256(layer[i] + (layer[i + 1] if i + 1 < len(layer) else layer[i]))
                     for i in range(0, len(layer), 2)]
        merkle_root = layer[0].hex()
        payload = BlockPayload(
            thought_type="meta",
            source=self._CONVERSATION_ANCHOR_SOURCE,
            content={
                "kind": "conversation_legacy_anchor",
                "spec": "Phase 14 §3K.3.C / INV-Syn-26",
                "legacy_fork_id": FORK_CONVERSATION,
                "legacy_fork_name": self.resolve_fork_name(FORK_CONVERSATION),
                "new_conversation_fork_id": conv_id,
                "chat_block_count": len(chat),
                "height_range": [min(heights), max(heights)],
                "merkle_root": merkle_root,
                "chat_block_hashes": [h.hex() for h in hashes],
                "tag_markers": list(self._CONVERSATION_CHAT_MARKERS),
                "note": ("Historical chat misrouted into the legacy fork before "
                         "INV-Syn-26. Additive provenance — never deleted/moved."),
            },
            significance=1.0,
            confidence=1.0,
            tags=["conversation_legacy_anchor", "conversation", "phase14"],
        )
        refs = [CrossRef(fork_id=FORK_CONVERSATION, block_height=min(heights)),
                CrossRef(fork_id=FORK_CONVERSATION, block_height=max(heights))]
        block = self.commit_block(
            fork_id=conv_id, epoch_id=0, payload=payload, pot_nonce=1,
            chi_spent=0.0, neuromod_state={}, cross_refs=refs)
        if block:
            logger.info(
                "[TimeChain] conversation legacy anchor committed → fork %d "
                "height %d (%d legacy chat blocks, merkle %s, INV-Syn-26 §3K.3.C)",
                conv_id, block.header.block_height, len(chat), merkle_root[:16])
        return block
