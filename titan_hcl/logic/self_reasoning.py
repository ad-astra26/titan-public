"""
titan_hcl/logic/self_reasoning.py — Introspective Cognition for Titan.
...
class SelfReasoningEngine:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            # Track 2 D5 (v1.2.1) — observation counters from
            # self_reflection_worker bus subscribers.
            "observed_chains": getattr(self, "_observed_chains", 0),
            "observed_meta_chains": getattr(self, "_observed_meta_chains", 0),
            "last_observed_chain_ts": getattr(
                self, "_last_observed_chain_ts", 0.0),
            "last_observed_meta_ts": getattr(
                self, "_last_observed_meta_ts", 0.0),
        }

    # ── Track 2 D5 (v1.2.1) — observation surface from L2 worker dispatcher ──
    #
    # self_reflection_worker's _handle_reasoning_stats / _handle_meta_reasoning_stats
    # (chunk B5) used to only cache stats payloads in state_refs because these
    # methods didn't exist on the engine. Per rFP_phase_c_self_improvement_subsystem_migration
    # §2.B.3 + Prime Directive #1 closure, the engine now exposes a minimal
    # observation surface: counters + last-seen timestamps + cached payloads.
    # Deeper integration (using chain CONTENT for self-prediction calibration)
    # waits on a granular REASONING_COMMIT bus event with chain body — out
    # of Track 2 scope per the strategy rFP "1 session per worker" cadence.
    #
    # These methods are intentionally minimal + side-effect-bounded:
    #   • counter increment (so get_stats observability surfaces activity)
    #   • timestamp cache (so a stalled subscriber is detectable)
    #   • most-recent stats payload (for downstream introspection)

    def observe_chain(self, reasoning_stats: dict) -> None:
        """Note a reasoning-chain event from cognitive_worker (via the
        REASONING_STATS_UPDATED bus event proxy). Caller passes the bus
        payload's stats dict.

        Bumps `_observed_chains` counter + caches the latest stats + ts so
        introspective sub-modes (state_audit, prediction_review) can read
        the last cognitive-chain snapshot without RPC-querying cognitive_worker.
        """
        import time as _time
        self._observed_chains = getattr(self, "_observed_chains", 0) + 1
        self._last_observed_chain_ts = _time.time()
        self._last_reasoning_stats = dict(reasoning_stats) if isinstance(
            reasoning_stats, dict) else {}

    def observe_meta(self, meta_reasoning_stats: dict) -> None:
        """Note a META_REASONING_STATS_UPDATED event from cognitive_worker.
        Mirrors observe_chain for the meta-reasoning layer."""
        import time as _time
        self._observed_meta_chains = getattr(
            self, "_observed_meta_chains", 0) + 1
        self._last_observed_meta_ts = _time.time()
        self._last_meta_reasoning_stats = dict(meta_reasoning_stats) if isinstance(
            meta_reasoning_stats, dict) else {}
