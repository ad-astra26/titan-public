...
class VerifiedContextBuilder:
    ...
    def build(self, query: str, user_id: str = "",
        ...
        # Step 3: Query each store IN PARALLEL.
        # 2026-05-12: prior implementation iterated stores serially (one
        # sqlite3.connect per store, each ~1-2s under T3 Phase C WAL
        # contention — total ~8s for 4 stores). Stores are independent so
        # we fan out across a small thread pool. Each store's sync sqlite3
        # query runs in its own thread; we collect results in the original
        # store_key order so downstream ranking sees deterministic input.
