"""
titan_hcl/logic/meta_outer_context.py — Bridge Titan's inner and outer cognitive memory.
...
class XSearchReader:
    ...
    def search(self, query: str, count: int = 10) -> list[dict]:
        ...
        try:
            ...
            from titan_hcl.logic.social_x_gateway import BaseContext
...
# ── EventsPollReader (SHM-direct read of events_teacher_state.bin) ───────
#
# Phase C state-read unification (rFP §3 Phase C, 2026-05-18). Migrated from
# bus.request(EVENTS_WINDOW_POLL) — that message had no live handler and the
# Phase A §4.B.7 `events_teacher_state.bin` slot is the canonical SHM payload
# for current events-teacher window state (fingerprints_count, last_run_time,
# window_count, perception_buffer_size, follower_rotation_idx, mode_stats,
# felt_experiences, followers_tracked, windows_completed, ts). Per Preamble
# G18 (state transport is SHM only), polling state must read from SHM not bus.

class EventsPollReader:
    def __init__(self, bus: Any = None, timeout_s: float = 5.0):
        # `bus` + `timeout_s` retained for ctor-signature compat with
        # OuterContextReader wiring; reads are SHM-direct (no bus traffic).
        self._bus = bus
        self._timeout_s = float(timeout_s)
        ...
        try:
            ...
            from .session4_state_specs import EVENTS_TEACHER_STATE_SPEC
            titan_id = resolve_titan_id()
            shm_root = ensure_shm_root(titan_id)
            self._reader = StateRegistryReader(
                EVENTS_TEACHER_STATE_SPEC, shm_root)
        except Exception as e:
            logger.debug("[EventsPollReader] SHM init failed: %s", e)
    ...
    def poll_window(self) -> Optional[dict]:
        ...
        try:
            ...
            logger.debug("[EventsPollReader] SHM read err: %s", e)
            return None
        if raw is None:
            return None
        try:
            ...
            logger.debug("[EventsPollReader] msgpack decode err: %s", e)
            return None
