"""
titan_hcl/logic/language_pipeline.py — Pure Language Logic Functions.
...
def _reorder_vocab_by_perturbations(
    vocabulary: list,
    word_perturbations: dict,
) -> list:
    """Reorder vocabulary so words with non-zero perturbation weights come
    first (descending by abs weight); unperturbed words follow in original
    order. Preserves the full vocabulary — biases selection priority without
    excluding any candidates.

    Vocabulary entries can be plain strings OR tuples/dicts with a 'word' field
    (the legacy load_vocabulary format). We probe both shapes defensively.

    Per rFP §2.A.7 + D3 closure: composition_engine.selector picks earlier
    candidates with higher probability, so reordering = selection bias.
    """
    if not isinstance(vocabulary, list) or not vocabulary:
        return vocabulary

    def _extract_word(entry) -> str:
        ...
        if isinstance(entry, dict):
            w = entry.get("word") or entry.get("w")
            if isinstance(w, str):
                return w.lower()
        return ""

    # Split into (perturbed, unperturbed) preserving original order within each.
    perturbed: list[tuple[float, int, object]] = []
    unperturbed: list[tuple[int, object]] = []
    for idx, entry in enumerate(vocabulary):
        word = _extract_word(entry)
        weight = word_perturbations.get(word) if word else None
        if isinstance(weight, (int, float)) and weight != 0.0:
            perturbed.append((abs(float(weight)), idx, entry))
        else:
            unperturbed.append((idx, entry))

    if not perturbed:
        return vocabulary

    # Sort perturbed: descending by weight, ascending by original index on tie.
    perturbed.sort(key=lambda t: (-t[0], t[1]))
    return [e for _, _, e in perturbed] + [e for _, e in unperturbed]
...
def compose_sentence(
    ...
    max_level: int = 8,
    word_perturbations: dict | None = None,
) -> dict:
    """Compose a sentence from felt-state with DA-gated exploration + grammar.

    Source: spirit_worker.py lines 4090-4122.

    Track 2 D3 (v1.2.1): `word_perturbations` (dict[str, float]) — when
    provided by language_worker via WORD_PERTURBATION_HINT correlation
    (per SPEC §8.5 D-SPEC-38), the vocabulary is reordered so high-
    perturbation words appear first. This biases the composition engine's
    selector toward outer_interface_worker's narrator-recommended words
    WITHOUT excluding the full vocabulary (out-of-cache words still
    selectable, just lower priority). Closes deferral D3 from the
    Phase A+B audit — the actual SPEAK quality improvement is now
    ...
    # Track 2 D3 — vocabulary reorder by word_perturbations bias.
    if isinstance(word_perturbations, dict) and word_perturbations:
        vocabulary = _reorder_vocab_by_perturbations(vocabulary, word_perturbations)
    ...
    try:
        ...
        for raw_word in sentence.split():
            ...
            if row:
                ...
                # Backfill-forward (rFP §4.5: "existing rows degrade
                # gracefully until backfilled forward"). Cover BOTH the
                # first-transition case AND retroactive backfill of words
                # that were already 'producible' before Phase 1 deploy and
                # have grounded_at=0. COALESCE/NULLIF guards keep an
                # already-set grounded_at intact (felt-state at the FIRST
                # capture wins, not re-overwritten on every speak).
                # last_encountered marks the SPEAK event itself (every
                # touch advances watermark — Bug A closure 2026-05-13:
                # was missing here while update_word_learning at
                # inner_memory.py:577 sets it correctly, but SPEAK is
                # by far the most-frequent vocabulary touch path).
                now_ts = time.time()
                if grounded_summary:
                    get_client(caller_name="language_pipeline").write(
                        ...
                        "last_encountered = ?, "
                        "grounded_at = COALESCE(NULLIF(grounded_at, 0), ?), "
                        "grounded_felt_summary = "
                        "  COALESCE(NULLIF(grounded_felt_summary, ''), ?) "
                        "WHERE word=?",
                        (now_ts, now_ts, grounded_summary, word),
                        table="vocabulary",
                    )
                else:
                    get_client(caller_name="language_pipeline").write(
...
def apply_grounding_action_to_db(db_path: str, word: str, action,
    ...
    try:
        ...
        conn.close()
        from titan_hcl.persistence import get_client
        # apply_grounding_action is also a vocabulary touch — advance
        # last_encountered (Bug A closure 2026-05-13).
        get_client(caller_name="language_pipeline").write(
            "UPDATE vocabulary SET confidence=?, felt_tensor=?, "
            "cross_modal_conf=?, last_encountered=? WHERE word=?",
            (new_conf, json.dumps(ft), new_xm, time.time(), word),
            table="vocabulary",
        )
        return True
...
def update_language_stats(
    ...
    try:
        ...
        # Teacher sessions in last hour (per_hour) + last day (per_day —
        # SPEC §23.8 D-SPEC-87 Phase 3.F 24h smoothing for outer_mind
        # willing[14] exploration_drive).
        sessions = 0
        sessions_day = 0
        try:
        ...
        stats["teacher_sessions_last_day"] = sessions_day
