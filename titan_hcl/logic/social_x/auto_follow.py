...
Candidate source (2026-06-03 redesign — Maker-ratified): the high-relevance
NON-followed voices the topic searches surface. The author's ``user_id`` is
captured at SEARCH time (``felt_experiences.author_id`` — see
``events_teacher._fetch_topic_search``), so a freshly-discovered voice is
followable without any extra API call. The old design gated on recurrence ≥3×
and resolved ``user_id`` from ``community_registry`` — which structurally
excluded discovered voices (they recur sparsely and aren't in the registry), so
the policy was always dry. Now recurrence is a soft preference (rel≥0.8 is the
quality bar) and ``user_id`` comes straight from the captured column.

Policy knobs (config ``[social_x.auto_follow]``):
  - ``min_relevance``  (0.8): author must appear at relevance ≥ this …
  - ``min_recurrence`` (1): … at least N× in felt_experiences …
...
class AutoFollowPolicy:
    ...
    def run(self, *, titan_id: str, context, config: dict) -> int:
        ...
        if not af.get("enabled", False):
            return 0
        min_rec = int(af.get("min_recurrence", 1))
        ...
        followed = 0
        for author, seen_n, author_id, followers in candidates:
            ...
            if not a or a.lower() in self_handles:
                continue
            if not author_id:
                continue                      # not followable (no captured user_id)
            if self._is_following(a):
                continue                      # already follow them (curated/registry)
            if self._already_followed(titan_id, a):
                continue                      # never re-follow (dedup)
            res = self.gateway.follow(author_id, context, consumer="auto_follow",
                                      handle=a, source_id=str(seen_n))
            status = getattr(res, "status", "")
            if status == "posted":
                followed += 1
                logger.info("[auto_follow] followed @%s (uid=%s, seen %d× rel≥%.2f, "
                            "%d followers)", a, author_id, seen_n, min_rel, followers)
    ...
    def _recurring_high_relevance(self, *, titan_id, min_rel, window_s, min_rec):
        """High-relevance discovered authors WITH a captured user_id, ordered by
        recurrence then follower count. Returns (author, seen_n, author_id,
        followers). Rows without a captured author_id are excluded (unfollowable
        — e.g. legacy rows predating the capture)."""
        try:
            et = sqlite3.connect(self._et_db, timeout=5)
            et.row_factory = sqlite3.Row
            rows = et.execute(
                "SELECT author, count(*) n, max(author_id) author_id, "
                "       max(author_followers) followers FROM felt_experiences "
                "WHERE titan_id=? AND relevance >= ? AND created_at >= ? "
                "  AND author IS NOT NULL AND author != '' "
                "  AND author_id IS NOT NULL AND author_id != '' "
                "GROUP BY lower(author) HAVING n >= ? "
                "ORDER BY n DESC, followers DESC LIMIT 20",
                (titan_id, min_rel, time.time() - window_s, min_rec)).fetchall()
            et.close()
            return [(r["author"], int(r["n"]), str(r["author_id"] or ""),
                     int(r["followers"] or 0)) for r in rows]
        except Exception as e:
            logger.warning("[auto_follow] felt_experiences query failed: %s", e)
            return []

    def _is_following(self, author: str) -> bool:
        """True if community_registry marks this handle as already-followed."""
        try:
            sg = sqlite3.connect(self._sg_db, timeout=5)
            sg.row_factory = sqlite3.Row
            row = sg.execute(
                "SELECT is_following FROM community_registry "
                "WHERE lower(user_name)=lower(?)", (author,)).fetchone()
            sg.close()
            return bool(row and int(row["is_following"] or 0))
        except Exception:
            return False
