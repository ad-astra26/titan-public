...
class ComposedThoughtArchetype(ArchetypeBase):
    ...
    def _pool_a(self, *, now: float, cited_pairs: set[tuple[str, str]]) -> dict | None:
        ...
        try:
            ...
            # SCHEMA NOTE 2026-05-12: knowledge_concepts PK-name column is
            # `topic`, NOT `concept_name`. Prior `SELECT concept_name`
            # silently raised "no such column" → debug-logged → Pool A
            # always returned None → composed_thought never fired (verified
            # via dry-run probe + sqlite3 .schema knowledge_concepts:
            # id INTEGER PK, topic TEXT NOT NULL, …, associations TEXT
            # DEFAULT '[]', …). This is one of the 4 archetype bugs that
            # blocked rFP §4.8 gate-9 14-day acceptance.
            rows = conn.execute(
                "SELECT id, topic, associations, source, created_at, "
                "       summary "
