"""
titan_hcl/logic/inner_coordinator.py — Inner Trinity Coordinator (T3).
...
# Sentinel for set_dream_subsystems additive semantics (rFP_experience_
# distillation_phase_c) — distinguishes "not passed" from an explicit None.
_UNSET = object()
...
class InnerTrinityCoordinator:
    ...
    def __init__(
        ...
        # NOTE: _life_force retired per rFP §4.G + D-SPEC-57 (v1.8.3 2026-05-15).
        # LifeForceEngine now lives in life_force_worker subprocess; dreaming
        # transitions are signaled via DREAM_STATE_CHANGED bus event (which
        # life_force_worker subscribes to). set_dream_subsystems still accepts
        # life_force= kwarg for back-compat but ignores it.
        self._exp_orchestrator = None
        self._e_mem = None
        self._neuromod_system = None

        # rFP_dream_consolidation_suite_offtick_restoration (D-SPEC-105):
        # the full dream-consolidation suite (_on_dream_begin) is 10–73 s
        # host-variable — too slow to run inline on tick() without freezing
        # the epoch loop / tripping the Guardian heartbeat. It runs in an
        # off-tick daemon thread spawned on the BEGIN_DREAMING transition;
        # _on_dream_end (light, wake-side) runs in its own off-tick thread on
        # END_DREAMING. The single-flight lock + handle ensure at most one
        # consolidation in flight (dreams are well-spaced; defensive).
    ...
    def set_dream_subsystems(self, exp_orchestrator=_UNSET, life_force=None,
                             e_mem=_UNSET, neuromod_system=_UNSET):
        """Wire subsystems needed for dream side-effects.

        ADDITIVE (rFP_experience_distillation_phase_c, 2026-05-21): only sets the
        subsystems explicitly passed — a kwarg left at its sentinel default leaves
        the existing wiring untouched. Previously this clobbered ALL attrs to None,
        so the cognitive_worker boot ordering (neuromod wired at coordinator-init
        time, exp_orchestrator created later) left `_exp_orchestrator=None` →
        `_on_dream_begin` skipped `distill_cycle` → the Distill stage never ran.

        `life_force` kwarg accepted for back-compat but IGNORED per
        rFP §4.G + D-SPEC-57: dreaming transitions now flow via
        DREAM_STATE_CHANGED bus event (life_force_worker subscribes).
        """
        if exp_orchestrator is not _UNSET:
            self._exp_orchestrator = exp_orchestrator
        if e_mem is not _UNSET:
            self._e_mem = e_mem
        if neuromod_system is not _UNSET:
            self._neuromod_system = neuromod_system
        # life_force intentionally not stored (extracted to L2 worker)
        ...
        if self.dreaming and self.inner and self._last_dreaming_event != "GREAT_PULSE":
            try:
                ...
                if transition == "BEGIN_DREAMING":
                    ...
                    # FULL dream-consolidation suite (NS/reasoning/meta/
                    # autoencoder/mini/neuromod + experience distillation) runs
                    # OFF-TICK in a daemon thread spawned here, so neither the
                    # inherent ~10–15 s nor host-contention spikes (measured
                    # 58–73 s on T3) can freeze the epoch loop or trip the
                    # Guardian heartbeat (rFP_dream_consolidation_suite_offtick_
                    # restoration, D-SPEC-105). tick() returns immediately.
                    # Distillation is part of _on_dream_begin — no longer run
                    # inline here (one consolidation path, no double-distill).
                    self._spawn_dream_consolidation()
                    ...
                    # _on_dream_end (insight storage in e_mem + clustering
                    # telemetry + neuromod restoration) was orphaned on this
                    # live path along with _on_dream_begin. Restored off-tick:
                    # it waits for any in-flight begin-consolidation to finish
                    # (so insights/neuromod-reset land after the suite, never
                    # concurrently) then runs the wake-side side-effects.
                    self._spawn_dream_end(summary)
            except Exception as e:
    ...
    def _spawn_dream_consolidation(self) -> None:
        """Spawn the off-tick dream-consolidation thread on BEGIN_DREAMING.

        Runs the full `_on_dream_begin` suite (NS/reasoning/meta-wisdom/
        autoencoder/meta-reasoning/mini/neuromod + experience distillation)
        in a daemon thread so the consciousness epoch loop never blocks on
        the 10–73 s (host-variable) consolidation. Single-flight: a new
        BEGIN_DREAMING while a consolidation is still alive is skipped with a
        WARN (dreams are well-spaced — defensive only).

        rFP_dream_consolidation_suite_offtick_restoration §2.1 / D-SPEC-105.
        """
        with self._consolidation_lock:
            if (self._consolidation_thread is not None
                    and self._consolidation_thread.is_alive()):
                logger.warning(
                    "[Coordinator] Dream consolidation thread still running — "
                    "skipping new BEGIN_DREAMING spawn (single-flight guard)")
                return
            t = threading.Thread(
                target=self._consolidation_run,
            ...
            t.start()
            logger.info(
                "[Coordinator] Dream consolidation thread spawned (off-tick)")

    def _consolidation_run(self) -> None:
        """Thread body — the full `_on_dream_begin` consolidation suite.

        GIL note: the work is numpy (matmul/backprop) + file I/O, both of
        which release the GIL, so this runs genuinely concurrently with
        tick()'s Python work. Waking engine-training is gated off during the
        dream (cognitive_worker gates on is_dreaming), so the suite has
        de-facto exclusive access to the engines (Phase 0 §8).
        """
        try:
            self._on_dream_begin()
        except Exception as e:
            logger.error(
                "[Coordinator] Dream consolidation thread failed: %s",
                e, exc_info=True)

    def _spawn_dream_end(self, summary: Optional[dict]) -> None:
        """Spawn the off-tick `_on_dream_end` thread on END_DREAMING.

        Waits for any in-flight begin-consolidation to finish (so insight
        storage + neuromod-boost reset happen *after* the suite, never
        concurrently with it) then runs the wake-side side-effects. Kept
        off-tick so a slow `_e_mem.store_insight` (FAISS add) cannot stall
        the epoch loop. rFP §2.1 / D-SPEC-105.
        """
        ...
        def _run() -> None:
            try:
                ...
                logger.warning(
                    "[Coordinator] join begin-consolidation before "
                    "_on_dream_end failed: %s", e)
            try:
                self._on_dream_end(summary)
            except Exception as e:
                logger.error(
                    "[Coordinator] _on_dream_end thread failed: %s",
                    e, exc_info=True)

        with self._consolidation_lock:
            t = threading.Thread(
                target=_run, name="dream-end", daemon=True)
            self._consolidation_thread = t
            t.start()

    def join_consolidation(self, timeout: float = 15.0) -> None:
        """Block until any in-flight consolidation/dream-end thread finishes.

        Called on MODULE_SHUTDOWN so an in-flight `save_all` can complete
        (avoid a torn write) before the process exits. Daemon threads would
        otherwise be killed mid-save. rFP §2.3.
        """
        ...
        if t is not None and t.is_alive():
            try:
                ...
                logger.warning(
                    "[Coordinator] join_consolidation failed: %s", e)

    def _run_dream_distillation(self) -> None:
        """Experience-orchestrator dream distillation — the Distill stage of the
        Record→Distill→Bias loop (rFP_experience_distillation_phase_c).

        Extracted so the Phase C **live** dream driver `tick()` invokes it on
        BEGIN_DREAMING, not only the retired `_on_dream_begin` (which lives in the
        dead `coordinate()` path — `coordinate()` is referenced only by the
        heartbeat-stub `spirit_worker.py`). Without this, dreams ran via `tick()`
        but never consolidated recorded experiences into wisdom.

        PURE IN-PROCESS: `distill_cycle` clusters `pending_distillation=1` rows in
        `experience_orchestrator.db` and writes wisdom — no bus traffic. Fires
        once per dream onset (not per tick). No-op if the orchestrator isn't wired.
        """
        if not self._exp_orchestrator:
            return
        try:
    ...
    def _on_dream_begin(self) -> None:
        ...
        # 1. Life force dreaming state — RETIRED per rFP §4.G + D-SPEC-57.
        # life_force_worker subscribes to DREAM_STATE_CHANGED (emitted by
        # dream_state_worker on the dream_start edge per §4.I) and calls
        # LifeForceEngine.set_dreaming(True) in its own process. No work
        # needed here; phase_times entry preserved as 0.0 for telemetry
        # symmetry until G14 cleanup removes it.
        _phase_times["life_force"] = 0.0
        _t = time.perf_counter()
        ...
        # 3. Experience Orchestrator dream distillation (shared helper —
        # also invoked by the live tick() dream path per
        # rFP_experience_distillation_phase_c).
        self._run_dream_distillation()
    ...
    def _on_dream_end(self, summary: dict | None) -> None:
        ...
        # 5. Life force wake state — RETIRED per rFP §4.G + D-SPEC-57.
        # life_force_worker subscribes to DREAM_STATE_CHANGED (dream_end
        # edge) and calls LifeForceEngine.set_dreaming(False) in its own
        # process. No work needed here.
