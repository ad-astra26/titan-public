...
# Project root (four levels up: helpers/ → agency/ → logic/ → titan_hcl/ → project root)
_PROJECT_ROOT = Path(__file__).resolve().parents[4]
_TITAN_PLUGIN = _PROJECT_ROOT / "titan_hcl"
...
class CodeKnowledgeHelper:
    ...
    def _inspect_structure(self) -> dict:
        """Return file tree of titan_hcl/ with line counts and docstrings."""
        lines = []
        ...
        result = f"titan_hcl/ structure ({len(lines)} files):\n" + "\n".join(lines)
        return {
            "success": True,
            "result": result[:3000],
    ...
    def _read_source(self, file_path: str, page: int = 1) -> dict:
        ...
        # Try titan_hcl/ first, then scripts/
        resolved = None
