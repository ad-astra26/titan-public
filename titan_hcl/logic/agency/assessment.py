"""
titan_hcl/logic/agency/assessment.py — IQL Self-Assessment (Step 7.6).
...
class SelfAssessment:
    ...
    def __init__(self, llm_fn=None):
        ...
        """
        self._llm_fn = llm_fn
        # rolling_state_persistence — assessments survive Titan restarts
        # so trinity dim formulas reading `assessment.average_score /
        # trend / variance` warm from real history at boot instead of
        # cold-starting at 0. Closes the "T3 restart wipes 12 PARTIAL
        # dims" class identified 2026-05-12. Per-titan file:
        # `<titan_data_dir>/dim_history/assessment_history.json`.
        #
        # max_age_s=None on purpose (2026-05-19, trinity-130d session):
        # SelfAssessment computes the running mean/trend over the most
        # recent N assessments — entry COUNT is the rolling window, not
        # wall-clock age. A 24h filter (prior behavior) silently wiped
        # the deque whenever assess() didn't fire for a day → fleet-wide
        # outer_spirit + outer_mind + inner_spirit dims fell back to
        # SPEC_DEFAULT (16 PARTIAL dims on T1+T3 traced to this bug).
        # The max_entries=100 cap still bounds disk + memory usage.
        try:
            ...
            self._store = RollingStateStore(
                ...
                max_entries=100,
                max_age_s=None,  # cap is by entry count, not age
                save_every_n=1,  # save on every assessment (low rate)
                save_every_s=30.0,
                ts_key="ts",
            )
            self._assessments: list[dict] = self._store.load()
            if self._assessments:
                logger.info(
                    "[SelfAssessment] loaded %d persisted assessments from %s",
                    len(self._assessments), self._store.path)
        except Exception as _e:
            logger.warning(
                "[SelfAssessment] persistence boot failed (%s) — "
                "in-memory only", _e)
            self._store = None
            self._assessments = []
    ...
    async def assess(self, action_result: dict) -> dict:
        ...
        # Persist for warm-restart per rolling_state_persistence.
