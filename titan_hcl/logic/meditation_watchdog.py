"""
titan_hcl/logic/meditation_watchdog.py — Self-healing meditation cadence watchdog.
...
class MeditationWatchdog:
    ...
    The owning spirit_worker is responsible for:
      ...
      (F1/F2 cadence force-trigger retired per D-SPEC-107 — cadence owned by the
       meditation onset-v2 hard floor; this watchdog is detection + F3/F6 stuck
       recovery + F5 backup-trigger checks.)
    ...
    # classify_overdue() REMOVED (rFP_meditation_emergent_onset_redesign /
    # D-SPEC-107): the drain_flat∧gaba_flat heuristic gated the F1/F2
    # force-trigger and never fired for the live drain/GABA regime
    # (drain≈0.98 not flat). Cadence is now owned by the onset-v2 hard floor
    # (`max_interval_seconds`), so the watchdog F1/F2 path is detection-only and
    # this classifier is no longer needed. F3/F6 stuck-detection is unaffected.
