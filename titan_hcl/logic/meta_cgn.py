...
class MetaCGNConsumer:
    ...
    def __init__(self, send_queue=None, titan_id: str = "T1",
                 ...
                 ema_decay_gamma: float | None = None):
        ...
        # K1 (2026-05-20): per-observation Beta forgetting horizon. Config-driven
        # so the V-saturation lever (γ 0.999→0.99, horizon ~1000→~100) is tunable
        # without code edits. γ=1.0 → no decay (pre-B.1 behavior); γ<1.0 → forget
        # old evidence at rate 1/(1-γ) so V can differentiate. See
        # ARCHITECTURE_cgn_family §15.7 + PLAN_cgn_k1_gamma_canary_and_dna_regression.md.
        # Precedence: explicit ctor arg (tests / callers) > [meta_cgn] config
        # > COMPOSITION_DEFAULTS code default.
        self._ema_decay_gamma = float(
            ema_decay_gamma if ema_decay_gamma is not None
            else _mc_cfg.get("ema_decay_gamma",
                             COMPOSITION_DEFAULTS.get("ema_decay_gamma", 0.999)))
    ...
    def update_primitive_V(self, primitive_id: str, quality: float,
        ...
        try:
            ...
            ema_gamma = self._ema_decay_gamma
            ...
            # RFP_meta-reasoning_CGN_FIX.md §6 — cross-primitive V spread feeds
            # β-dispersion EMA. Breaks the shadow_mode deadlock: rerank-based
            # dispersion at line 2514-2519 only fires from chain_iql integration
            # with ≥2 candidates AND ramps composed_V via w_grounded which is
            # 0 in shadow_mode → composed_V dispersion = 0 → EMA stuck at 0 →
            # E3 threshold (0.015) never crossed → status stays shadow_mode.
            # Cross-primitive V spread (max-min across the 9 primitives) is
            # non-zero even in shadow_mode (T1 live 2026-05-16: 0.036).
            # Sourcing dispersion from primitive-V spread breaks the loop
            # without touching graduation gates or rerank logic.
            try:
                _v_vals = [
                    float(getattr(_pp, "V", 0.5))
                    for _pp in self._primitives.values()
                ]
                if len(_v_vals) >= 2:
                    _v_spread = max(_v_vals) - min(_v_vals)
                    _a = self._beta_dispersion_alpha
                    self._beta_dispersion_ema = (
                        (1 - _a) * self._beta_dispersion_ema
                        + _a * _v_spread)
            except Exception:
                pass  # graceful — never block update_primitive_V
        except Exception as e:
    ...
    def get_haov_stats(self) -> dict:
        ...
        # SPEC §23.9 / rFP_phase_c_130d_rust_l1_port §4.3.2 — `verified_rules`
        # is the count of confirmed hypotheses, consumed by outer_spirit
        # CHIT[5] situation_recognition + CHIT[13] causal_attribution
        # + ANANDA[5] information_accuracy + ANANDA[9] discovery_value.
        # Exposed at producer so both Phase A+B Python L2 and Phase C Rust L1
        # read the same SPEC-named key (previously both defaulted to 0
        # because the producer published only `by_status` nested dict).
        verified_rules = by_status.get("confirmed", 0)
        # `verified_rules_high_conf` per SPEC §23.9 ANANDA[5]: count of
        # confirmed hypotheses with confidence_multiplier ≥ 1.2 (strong
        # effect, not just marginal confirmation).
        verified_high_conf = sum(
            1 for h in self._hypotheses.values()
            if h.status == "confirmed" and h.confidence_multiplier >= 1.2
        )
        return {
            "total": len(self._hypotheses),
            "by_status": by_status,
            "verified_rules": verified_rules,
            "verified_rules_high_conf": verified_high_conf,
            "details": details,
        }
    ...
    def compose_template_score(self, template_id: str, state_ctx: dict,
        ...
        try:
            ...
            if Q_conf + V_confidence > 1e-6:
                ...
                lam = 0.5
            # λ emerges from the confidence ratio above — no hard override.
            # 2026-05-20 (K1 + composition-weight): removed the prior
            # `if V_confidence<0.2 and Q_conf>0.5: lam=max(lam,0.7)` forced
            # floor. It was redundant (the ratio Q_conf/(Q_conf+V_conf) already
            # yields ~0.71 in that regime) and its only lasting effect was to
            # PIN λ high — keeping META-CGN's (correct, anti-FORMULATE) vote
            # clamped to ≤0.3 even as V_confidence grew. Letting λ float lets
            # β's influence rise organically as K1 (γ=0.99) un-saturates V and
            # V_confidence climbs. [0.1, 0.9] retained as pure safety bounds.
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            # SPEC §23.8 / rFP_phase_c_130d_rust_l1_port §3.1 — per-hour
            # rate consumed by outer_mind thinking[14] exploration_drive.
            "eureka_accelerated_per_hour": round(
                self._eureka_accelerated_updates
                / max(1.0, (time.time() - self._start_ts) / 3600.0),
                4,
            ),
            # SPEC §23.8 D-SPEC-87 (2026-05-18 Phase 3.F wave 3a) — 24h
            # smoothing variant consumed by outer_mind willing[14]
            # exploration_drive (denom 5 → 120 in Rust outer-mind-rs).
            "eureka_accelerated_per_day": round(
                self._eureka_accelerated_updates
                / max(1.0, (time.time() - self._start_ts) / 86400.0),
                4,
            ),
            ...
            # SPEC §23.8 / rFP_phase_c_130d_rust_l1_port §3.1 — aggregated
            # ratio consumed by outer_mind thinking[0] research_effectiveness
            # + thinking[1] knowledge_retrieval and outer_spirit CHIT[0,6] +
            # ANANDA[5,9]. Sum of helpful counts across sources / total
            # responses received.
            "knowledge_helpful_ratio": round(
                min(1.0, max(0.0,
                    sum(self._knowledge_helpful_by_source.values())
                    / max(1, self._knowledge_responses_received))),
                4,
            ),
