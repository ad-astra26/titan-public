"""RestoreWorker — the ONE cold+warm restore pipeline (RFP_backup_redesign_spine §7.C).

Relocated VERBATIM from scripts/backup_restore_sovereign.py — the proven 5J-3
sovereign resurrection engine (INV-MBR-0/12: "a Maker restores their Titan with
ONLY Shard-1"). The engine logic (resurrect_from_chain / restore_body_from_chain
/ the streamed fetch→verify(arc+mrkl)→apply) is UNCHANGED — a byte-identical move
into the logic layer so the live module + the standalone scripts share ONE
engine (INV-BRS-8). RestoreWorker wraps it with a unified restore(*, mode) entry
+ a process-wide single-flight guard (INV-BRS-6 — closes the audit H-bug that the
weekly restore-test was not lock-guarded). The apply goes through
apply_event_components (== what DiffEngine.apply wraps).
"""
...
from titan_hcl.logic.backup_restore import apply_event_components
...
class RestoreWorker:
    """Unified cold+warm restore pipeline (RFP §7.C). Wraps the proven sovereign
    engine (restore_body_from_chain) byte-identical; adds a process-wide
    single-flight guard (INV-BRS-6) + an explicit mode. The engine is sync
    (asyncio.run internally), so restore() is sync — the cold scripts + the warm
    stager thread are both sync contexts.

    mode:
      "full"        → reconstruct + atomically COMMIT into install_root/data
                       (cold resurrection / live recovery).
      "verify_test" → reconstruct into a scratch dir, NO commit (the §24.12
                       Sunday byte-for-byte test compares scratch vs live).
    """
    _restore_lock = threading.Lock()

    def __init__(self, *, titan_id: str, network: str = "mainnet",
                 rpc_url: Optional[str] = None) -> None:
        self.titan_id = titan_id
        self.network = network
        self.rpc_url = rpc_url

    def restore(self, *, key_bytes: bytes, titan_pubkey: str, install_root: str,
                mode: str = "full", scratch_dir: Optional[str] = None,
                best_effort: bool = False, verbose: bool = True) -> ResurrectionResult:
        if mode not in ("full", "verify_test"):
            raise ValueError(f"mode must be 'full' or 'verify_test', got {mode!r}")
        commit = (mode == "full")
        if not RestoreWorker._restore_lock.acquire(blocking=False):
            out = ResurrectionResult(titan_id=self.titan_id)
            out.status = "halted"
            out.halt_reason = "restore_already_in_flight"
            out.errors.append(
                "another restore is in flight (single-flight, INV-BRS-6)")
            return out
        try:
            return restore_body_from_chain(
                key_bytes=key_bytes, titan_pubkey=titan_pubkey,
                titan_id=self.titan_id, install_root=install_root,
                scratch_dir=scratch_dir, rpc_url=self.rpc_url,
                network=self.network, commit=commit,
                best_effort=best_effort, verbose=verbose)
        finally:
