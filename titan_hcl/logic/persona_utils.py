...
async def score_llm_quality(response: str, persona_message: str,
    ...
    (llm_base_url, llm_api_key, llm_model) → (api_base, internal_key, model)
    so calls route through POST {api_base}/v4/llm-distill instead of
    direct httpx to ollama.com/v1. Per the architectural rule, out-of-
    process scripts (persona_endurance, persona_social_v2) use the HTTP
    proxy endpoint. Latency: ~1-3s (same as before; just one extra HTTP
    hop through api_subprocess → kernel_rpc → llm_worker → provider).
    """
    if not response or not api_base or not internal_key:
        return None
    ...
    try:
        ...
        text = await distill_via_http_async(
            text=prompt,
            instruction="",
            api_base=api_base,
            ...
            max_tokens=10,
            temperature=0.1,
            consumer="persona_score_llm_quality",
            timeout_s=15.0,
        )
        if text:
