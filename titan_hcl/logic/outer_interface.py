"""
titan_hcl/logic/outer_interface.py — Outer Interface: ACTION→REACTION→OBSERVATION.
...
class OuterInterface:
    ...
    # ── Durable persistence (data/outer_interface_state.json per §11.H.1) ──
    _STATE_PATH = "data/outer_interface_state.json"

    def save_state(self, path: str | None = None) -> None:
        """Atomically persist get_state() to disk (§11.H.2 tmp+os.replace).

        AUDIT §C fix (rFP §P2): this method DID NOT EXIST — the worker's
        shutdown `hasattr(outer_interface, "save_state")` guard was therefore
        always False → a NOP, so advisor refractory + composition + mode/stats
        were lost on every respawn (and boot never restored them). Pairs with
        load_state(), now wired on boot. get_state() is JSON-clean.
        """
        import json
        import os as _os
        ...
        try:
            ...
            logger.warning("[OuterInterface] save_state failed: %s", e)

    def load_state(self, path: str | None = None) -> None:
        """Restore persisted state on boot (pairs with save_state). No-op if
        the file is absent (fresh install / first boot)."""
        import json
        p = path or self._STATE_PATH
        try:
            with open(p) as f:
            ...
            logger.warning("[OuterInterface] load_state failed: %s", e)
