"""
titan_hcl/logic/inner_memory.py — Dual-Layer Inner Memory System.
...
class InnerMemoryStore:
    ...
    def __init__(self, db_path: str = "./data/inner_memory.db"):
        ...
        # Resolve writer client. If our db_path differs from the IMW global
        # default (any test using tmp_path / any future per-Titan-isolated
        # store), use a per-instance writer-config pinned to OUR db_path —
        # otherwise writes silently land in the wrong file (production-mode
        # only "works" because every live caller uses the default path).
        # Closes the 13 test_language_learning.py false-negatives that
        # surfaced 2026-05-13 (BUG-INNER-MEM-STORE-DB-PATH-DRIFT).
        default_client = get_client(caller_name="inner_memory_store")
        try:
            default_db = str(Path(default_client._cfg.db_path).resolve())
        except Exception:
            default_db = ""
        our_db = str(Path(db_path).resolve())
        if our_db == default_db:
            self._client = default_client
        else:
            # Non-default path → use a per-path singleton with cfg pinned
            # to OUR db_path. Stays direct-mode if global is disabled
            # (tests via conftest); otherwise still routes via IMW with
            # the correct target DB.
            from titan_hcl.persistence.config import IMWConfig
            cfg = IMWConfig.from_titan_config()
            cfg = replace(cfg, db_path=our_db)
            self._client = get_client(
                caller_name=f"inner_memory_store::{our_db}", cfg=cfg)
        self._init_schema()
