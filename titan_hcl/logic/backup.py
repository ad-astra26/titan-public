...
class RebirthBackup:
    ...
    async def commit_event_v3_chain(
        ...
        for i, comp in enumerate(components):
            try:
                memo = build_v3_memo(
                    event_id=event_id, ts=int(ts), tier=comp["tier"],
                    archive_hash=comp["arc"], merkle_root=event_merkle_root,
                    arweave_tx=comp["tx_id"], mode=mode,
                    prev_sig=prev_sig, url_key=url_key,
                )
            except Exception as e:
    ...
    async def _run_unified_event_v2(self, weekday: int) -> bool:
        ...
        async def _v3_chain_commit(event_id: str, ts: int, event_root: str,
                                   components: list, prev_sig) -> Optional[dict]:
            return await self.commit_event_v3_chain(
                event_id=event_id, ts=ts, event_merkle_root=event_root,
                components=components, prev_sig=prev_sig,
            )
