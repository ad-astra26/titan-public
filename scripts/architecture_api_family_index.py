#!/usr/bin/env python3
"""ARCHITECTURE_api_family_index.md generator — auto-built TOC + §19 invariants ledger + §20 considered-rejected ledger.

Mirrors `scripts/architecture_synthesis_engine_index.py` discipline for the
API-family architecture narrative doc:
...
Output: titan-docs/specs/ARCHITECTURE_api_family_index.md (overwrites; never hand-edit).
...
ARCH_PATH = REPO_ROOT / "titan-docs" / "specs" / "ARCHITECTURE_api_family.md"
INDEX_PATH = REPO_ROOT / "titan-docs" / "specs" / "ARCHITECTURE_api_family_index.md"
...
def render(parsed: dict) -> str:
    out: list[str] = []
    out.append("# ARCHITECTURE — API Family Index (auto-generated)")
    out.append("")
    out.append(
        "> **Auto-generated** from `titan-docs/specs/ARCHITECTURE_api_family.md` by "
        "`scripts/architecture_api_family_index.py`. **DO NOT EDIT** — "
        "regenerate via `python scripts/architecture_api_family_index.py` "
    ...
    out.append("")
    out.append(
        "> **Read this file FIRST when working on the API / Observatory-readout "
        "layer — routes (/v1, /v3, /v4), the L3 read abstraction (TitanStateAccessor "
        "+ ShmReaderBank + cache_schemas + dim_registry + state_mapping), api_subprocess, "
        "WebSocket/OBSERVATORY_EVENT/CHAT contracts, the v6 consolidation.** Pair with "
        "the §0 Changelog at the top of `ARCHITECTURE_api_family.md`. Implementation "
        "proposals + phasing live in `RFP_phase_c_titan_hcl_cleanup.md`. Proto-SPEC-module: "
        "absorbs into a future `SPEC_api_family.md` at the SPEC modularization threshold."
    )
    out.append("")
    ...
    if parsed["invariants"]:
        ...
        out.append(
            ...
            "INV-1..6 restate main-SPEC Preamble invariants (G18–G22, G-RPC-5) at "
            "the API layer where they are most often violated — they backlink, "
    ...
    out.append("")
    out.append(
        "_Regenerate with `python scripts/architecture_api_family_index.py` "
