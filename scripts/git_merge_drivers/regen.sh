...
case "$INDEX_KIND" in
    SPEC_index)
        # spec_index.py reads titan-docs/SPEC_titan_architecture.md and
        # writes titan-docs/SPEC_index.md. Since git places the working
        # version at OURS_PATH, we redirect the output there.
        python scripts/spec_index.py > /dev/null 2>&1
        cp titan-docs/SPEC_index.md "$OURS_PATH"
        ;;
    OBSERVABLES_index)
    ...
    ARCHITECTURE_cgn_family_index)
        # architecture_cgn_family_index.py reads titan-docs/ARCHITECTURE_cgn_family.md
        # and writes titan-docs/ARCHITECTURE_cgn_family_index.md.
        python scripts/architecture_cgn_family_index.py > /dev/null 2>&1
        cp titan-docs/ARCHITECTURE_cgn_family_index.md "$OURS_PATH"
        ;;
    ARCHITECTURE_synthesis_engine_index)
        # architecture_synthesis_engine_index.py reads titan-docs/ARCHITECTURE_synthesis_engine.md
        # and writes titan-docs/ARCHITECTURE_synthesis_engine_index.md.
        python scripts/architecture_synthesis_engine_index.py > /dev/null 2>&1
        cp titan-docs/ARCHITECTURE_synthesis_engine_index.md "$OURS_PATH"
        ;;
    ARCHITECTURE_api_family_index)
        # architecture_api_family_index.py reads titan-docs/ARCHITECTURE_api_family.md
        # and writes titan-docs/ARCHITECTURE_api_family_index.md.
        python scripts/architecture_api_family_index.py > /dev/null 2>&1
        cp titan-docs/ARCHITECTURE_api_family_index.md "$OURS_PATH"
        ;;
    ARCHITECTURE_trinity_index)
        # architecture_trinity_index.py reads titan-docs/ARCHITECTURE_trinity.md
        # and writes titan-docs/ARCHITECTURE_trinity_index.md.
        python scripts/architecture_trinity_index.py > /dev/null 2>&1
        cp titan-docs/ARCHITECTURE_trinity_index.md "$OURS_PATH"
        ;;
    *)
