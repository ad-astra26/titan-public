...
# Phase C systemd detection — Phase C T1+T2 migration session 2026-05-14.
# When `titan-t1.service` is provisioned, defer to systemctl restart so
# Rust kernel-rs is the parent process (with 8 Rust daemons + Python
# plugin as supervised children, mirror of T2/T3). Pre-migration setups
# (no systemd unit) keep the legacy nohup-based restart logic intact.
#
# This branch runs AFTER the dream-state safety gate above, so the
# "never wake mid-dream" contract applies equally to systemd-mode and
# nohup-mode restarts.
if [[ "$TARGET" == "t1" ]] && systemctl list-unit-files titan-t1.service 2>/dev/null | grep -q '^titan-t1.service'; then
    echo ""
    echo "=== Restarting T1 (systemd) ==="
    sudo systemctl restart titan-t1.service
    # Wait briefly for boot then probe /health
    sleep 12
    HTTP=$(curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:7777/health 2>/dev/null)
    if [[ "$HTTP" == "200" ]]; then
        echo "T1 health: OK (200)"
        exit 0
    else
        echo "T1 health: PENDING ($HTTP) — may need more boot time"
        exit 0
    fi
fi

# Perform the restart (legacy nohup path)
if [[ "$TARGET" == "t1" ]]; then
    echo ""
    echo "=== Restarting T1 ==="
    ...
    # 1. Identify titan_hcl parent process group and signal the WHOLE GROUP.
    #    Microkernel v2 amendment 2026-04-26: previously only matched
    #    "titan_hcl" in cmdline, but Guardian's spawned workers (api_subprocess,
    ...
    #    DON'T mention titan_hcl. SIGKILL hit only the parent, leaving 5-10
    ...
    PARENT_PIDS=$(ps aux | grep titan_hcl | grep python | grep -v grep | awk '{print $2}')
    PGIDS=""
    ...
    for i in $(seq 1 50); do
        if ! pgrep -f "python.*titan_hcl" >/dev/null 2>&1; then
            break
        fi
        sleep 0.3
    done
    ...
    if pgrep -f "python.*titan_hcl" >/dev/null 2>&1; then
        echo "  ⚠  titan_hcl processes still alive after 15s SIGTERM — escalating to SIGKILL"
        if [[ -n "$PGIDS" ]]; then
            for pgid in $PGIDS; do
                kill -KILL -$pgid 2>/dev/null
            done
        fi
        # Also kill any titan_hcl parent that survived (paranoid)
        ps aux | grep titan_hcl | grep python | grep -v grep | awk '{print $2}' | xargs -r kill -9 2>/dev/null
    ...
    #     don't show up in `pgrep titan_hcl` but consume RAM until reaped.
    ...
    #    a dead parent.
    rm -f data/titan_hcl.pid 2>/dev/null
    ...
    # TITAN_API_PORT honored by api_subprocess at boot (titan_hcl.py:267-269).
    MALLOC_ARENA_MAX=2 OPENROUTER_API_KEY='' TITAN_API_PORT="$ACTIVE_PORT" nohup python -u scripts/titan_hcl.py --server >> /tmp/titan_brain.log 2>&1 &
    NEW_PID=$!
