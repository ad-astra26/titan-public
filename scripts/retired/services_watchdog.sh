...
# ═══════════════════════════════════════════════════════════════════════
# CHECK 2: Teacher activity (runs inside titan_hcl)
# ═══════════════════════════════════════════════════════════════════════
if [ -f "$BRAIN_LOG" ]; then
...
# Count titan_hcl parent processes (should be exactly 1 per Titan)
# IMPORTANT: Filter by CWD matching PROJECT_DIR so T2/T3 on shared VPS
# don't kill each other's processes
_own_pids() {
    pgrep -f "titan_hcl.*--server" 2>/dev/null | while read pid; do
...
if [ "$TITAN_PARENTS" -gt 1 ] && [ "$RESTART_IN_FLIGHT" = "true" ]; then
    ...
    # ALL titan_hcl groups + the stale lock so the next watchdog cycle
    # restarts cleanly via the dead-titan branch.
    CURR_STATE[zombies]="orchestrator_crashed"
    ERRORS="${ERRORS}Zombies: stale shadow-swap lock + ${TITAN_PARENTS} duplicate titan_hcl groups — force-cleaning\n"
    ...
    ERRORS="${ERRORS}Zombies: ${TITAN_PARENTS} duplicate titan_hcl groups detected — killing extras\n"
