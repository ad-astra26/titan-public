...
def main() -> int:
    ...
    p.add_argument(
        ...
        help="Pre-commit mode: exit 1 if closed entries still live in "
             "BUGS.md (split is stale). Tells user to run the script "
             "manually + git add the result."
    )
    args = p.parse_args()
    ...
    if GRAVEYARD.exists():
        ...
        for i, ln in enumerate(prev):
            # Match BOTH `### BUG-X —` and `### ~~BUG-X~~ —` (graveyard
        ...
        if added_rows or added_bodies:
            ...
            for i, ln in enumerate(grave_lines):
                if ln.startswith("> **Closed entries:**"):
                    grave_lines[i] = f"> **Closed entries:** {len(all_closed_slugs)}  ·  Generated by `scripts/split_bugs_graveyard.py`."
                    break

    if args.check:
        if len(closed_slugs) > 0:
            print(
                f"BUGS_graveyard split is STALE: {len(closed_slugs)} closed entries "
                f"still in BUGS.md (should be in BUGS_graveyard.md).",
                file=sys.stderr,
            )
            print(
                f"Fix: python scripts/split_bugs_graveyard.py "
                f"&& git add titan-docs/BUGS.md titan-docs/BUGS_graveyard.md",
                file=sys.stderr,
            )
            return 1
        return 0
