#!/usr/bin/env python3
"""ARCHITECTURE_trinity_index.md generator — auto-built TOC + §19 invariants ledger + §20 considered-rejected ledger.

Mirrors `scripts/architecture_api_family_index.py` discipline for the
Trinity architecture narrative doc:
...
Output: titan-docs/specs/ARCHITECTURE_trinity_index.md (overwrites; never hand-edit).
...
ARCH_PATH = REPO_ROOT / "titan-docs" / "specs" / "ARCHITECTURE_trinity.md"
INDEX_PATH = REPO_ROOT / "titan-docs" / "specs" / "ARCHITECTURE_trinity_index.md"


SECTION_RE = re.compile(r"^(#{2,4})\s+(§[\w\.]+)\s*[—-]\s*(.+?)\s*$")
# §19 invariants table row: | INV-3 | <text> | <status> | <line-ref> |
INVARIANT_ROW_RE = re.compile(r"^\|\s*(INV-\d+)\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|\s*$")
# §20 rejected bullet: - **<pattern>** <rest — reason>
REJECTED_RE = re.compile(r"^-\s+\*\*(.+?)\*\*\s*(.+?)\s*$")
# maturity tag suffix on a header title:  `[IMPLEMENTED]` / `[TARGET]` / `[meta]` / `[ledger]` ...
...
def render(parsed: dict) -> str:
    out: list[str] = []
    out.append("# ARCHITECTURE — Trinity Index (auto-generated)")
    out.append("")
    out.append(
        "> **Auto-generated** from `titan-docs/specs/ARCHITECTURE_trinity.md` by "
        "`scripts/architecture_trinity_index.py`. **DO NOT EDIT** — "
        "regenerate via `python scripts/architecture_trinity_index.py` "
    ...
    out.append("")
    out.append(
        "> **Read this file FIRST when working on the Trinity — the three symmetries "
        "+ power-of-three (5/15/45), the 162D nested hierarchy (130 trinity + 30 topology "
        "+ 2 journey), the per-trinity closed enrichment loop (filter_down/ground_up), the "
        "sphere clocks + BIG/GREAT pulse mechanic, spirit bathing/micro_enrich, and the "
        "homeostasis-restoration P0/P1 design.** Pair with the §0 Changelog at the top of "
        "`ARCHITECTURE_trinity.md` and the §6 source map in "
        "`NOTES_trinity_middle_path_homeostasis.md`. Proto-SPEC-module: absorbs into a "
        "future `SPEC_trinity.md` at the SPEC modularization threshold."
    )
    out.append("")
    ...
    if parsed["invariants"]:
        ...
        out.append(
            ...
            "These restate / localize main-SPEC Trinity invariants (G1–G11, §7.1, "
            "§9.A) at the Trinity layer — they backlink, they do not redefine."
        )
        out.append("")
    ...
    out.append("")
    out.append(
        "_Regenerate with `python scripts/architecture_trinity_index.py` "
