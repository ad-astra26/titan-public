...
def tool_present_at(tool: str, version: str) -> ToolStatus:
    ...
    # anchor is avm-managed: the `anchor` shim (~/.cargo/bin/anchor → avm) does a
    # one-time latest-version check on its FIRST post-install invocation, so it
    # briefly misreports (e.g. "2.3.0" / unparseable) right after `avm use`. Verify
    # against the DETERMINISTIC avm-installed binary ~/.avm/bin/anchor-<version>
    # instead; fall through to the PATH shim only when avm isn't managing it.
    if tool == "anchor":
        avm_bin = Path.home() / ".avm" / "bin" / f"anchor-{version}"
        if avm_bin.exists():
            found = _extract_version(_probe_version(str(avm_bin)) or "")
            ok = _satisfies(version, found)
            return ToolStatus(tool, version, exe, present=True, found=found,
                              compatible=ok,
                              detail=(f"avm anchor-{version} ({found})" if ok
                                      else f"avm binary anchor-{version} present but reports {found or '?'}"))
