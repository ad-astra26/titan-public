...
Hosted in ``titan_HCL`` (today's ``titan_main`` parent) as an asyncio
...
──────────────────────────────────────────────────────────────────────
Refresh period = ``OUTER_BODY_TICK_BASE_S`` (10.0 s, matching the
daemon's fastest natural cadence per SPEC §18.1).
...
REFRESH_PERIOD_S: float = OUTER_BODY_TICK_BASE_S  # 10.0
