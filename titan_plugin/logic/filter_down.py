...
# ── V4 FilterDown Engine (30-dim Full Trinity) ──────────────────────────

class FilterDownV4Engine:
    """
    V4 extension: 30-dim FILTER_DOWN covering both Inner and Outer Trinity.

    Uses a larger network (30→64→32→1) to learn attention across the full
    30DT SPIRIT tensor. Produces severity multipliers for:
      - inner_body[5], inner_mind[5] (same as V3 — sent to Body/Mind workers)
      - outer_body[5], outer_mind[5] (new — sent to Core for agency tuning)
      - Spirit dimensions (inner[10:15] + outer[25:30]) are NOT published

    Wraps a separate TrinityValueNet and TransitionBuffer with V4-specific
    weight/buffer file paths. The V3 FilterDownEngine continues to run
    ...
    def __init__(self, config: Optional[dict] = None, data_dir: str = "./data"):
        ...
        import numpy as np
        self._net.w1 = np.random.randn(V4_STATE_DIM, V4_HIDDEN_1).astype(np.float64) * math.sqrt(2.0 / V4_STATE_DIM)
        self._net.b1 = np.zeros(V4_HIDDEN_1, dtype=np.float64)
        self._net.w2 = np.random.randn(V4_HIDDEN_1, V4_HIDDEN_2).astype(np.float64) * math.sqrt(2.0 / V4_HIDDEN_1)
        self._net.b2 = np.zeros(V4_HIDDEN_2, dtype=np.float64)
        ...
        self._buffer_path = os.path.join(data_dir, "filter_down_v4_buffer.json")

        # Multipliers for 4 observable layers (5 dims each)
        self._inner_body_multipliers = [1.0] * 5
        self._inner_mind_multipliers = [1.0] * 5
        self._outer_body_multipliers = [1.0] * 5
        self._outer_mind_multipliers = [1.0] * 5
        ...
        self._net.load(self._weights_path)
        self._buffer.load(self._buffer_path)
        logger.info("[FilterDownV4] Initialized: %d transitions, weights=%s",
                     len(self._buffer), "loaded" if os.path.exists(self._weights_path) else "random")

    def record_transition(self, spirit_tensor_30: list[float],
                          next_spirit_tensor_30: list[float]) -> None:
        """Record a 30DT SPIRIT tensor transition."""
        from .middle_path import middle_path_loss

        state = list(spirit_tensor_30)
        next_state = list(next_spirit_tensor_30)

        # Reward from BOTH trinities' middle path loss
        inner_loss = middle_path_loss(next_state[0:5], next_state[5:10], next_state[10:15])
        outer_loss = middle_path_loss(next_state[15:20], next_state[20:25], next_state[25:30])
    ...
    def maybe_train(self) -> Optional[float]:
        ...
        logger.info("[FilterDownV4] Train step %d: loss=%.6f buffer=%d",
                     self._total_train_steps, loss, len(self._buffer))
        return loss

    def compute_multipliers(self, spirit_tensor_30: list[float]) -> dict:
        """
        Compute severity multipliers from 30-dim attention gradients.

        Returns dict with:
          inner_body[5], inner_mind[5], outer_body[5], outer_mind[5]
        Spirit dims (inner[10:15], outer[25:30]) NOT included — Spirit observes.
        """
        if self._total_train_steps < 1:
            return {
                "inner_body": list(self._inner_body_multipliers),
                "inner_mind": list(self._inner_mind_multipliers),
                "outer_body": list(self._outer_body_multipliers),
                "outer_mind": list(self._outer_mind_multipliers),
            }

        grad = self._net.gradient_wrt_input(spirit_tensor_30)

        # Split gradients (absolute values for importance)
        ib_grad = [abs(g) for g in grad[0:5]]
        im_grad = [abs(g) for g in grad[5:10]]
        # inner_spirit grad[10:15] — not published
        ob_grad = [abs(g) for g in grad[15:20]]
        om_grad = [abs(g) for g in grad[20:25]]
        # outer_spirit grad[25:30] — not published

        # Normalize across all observable gradients
        all_grad = ib_grad + im_grad + ob_grad + om_grad
        max_grad = max(all_grad) if all_grad else 1.0
        if max_grad < 1e-8:
            max_grad = 1.0
        ...
        self._outer_body_multipliers = _ema(self._outer_body_multipliers, new_ob)
        self._outer_mind_multipliers = _ema(self._outer_mind_multipliers, new_om)

        return {
            "inner_body": [round(m, 4) for m in self._inner_body_multipliers],
            "inner_mind": [round(m, 4) for m in self._inner_mind_multipliers],
            "outer_body": [round(m, 4) for m in self._outer_body_multipliers],
            "outer_mind": [round(m, 4) for m in self._outer_mind_multipliers],
        }

    def get_stats(self) -> dict:
        return {
            "version": "v4",
            "state_dim": V4_STATE_DIM,
            "buffer_size": len(self._buffer),
            "total_train_steps": self._total_train_steps,
            "last_loss": round(self._last_loss, 6),
            "inner_body_multipliers": [round(m, 4) for m in self._inner_body_multipliers],
            "inner_mind_multipliers": [round(m, 4) for m in self._inner_mind_multipliers],
            "outer_body_multipliers": [round(m, 4) for m in self._outer_body_multipliers],
            "outer_mind_multipliers": [round(m, 4) for m in self._outer_mind_multipliers],
        }
    ...
    def _persist(self) -> None:
        try:
            ...
            logger.warning("[FilterDownV4] Persist failed: %s", e)
