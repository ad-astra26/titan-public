...
Threading model
──────────────────────────────────────────────────────────────────────
Hosted INSIDE the body_worker subprocess (``titan_plugin.modules.body_worker``)
...
from titan_plugin._phase_c_constants import SCHUMANN_BODY_HZ
    ...
    def _run(self, stop_event: threading.Event) -> None:
        ...
        while not stop_event.is_set():
            ...
            if sleep_for > 0:
                ...
                # catch up at full rate.
                logger.warning(
                    "inner_body_sensor_refresh drift recovery: "
                    "behind by %.2fs (>%d periods); resetting cadence",
                    -sleep_for, _DRIFT_RECOVERY_PERIODS,
                )
                next_tick = time.monotonic()
