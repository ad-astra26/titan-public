...
const COLORS: Record<string, string> = {
  Sovereign: '#E5C79E',
  Collaborative: '#77CCCC',
  Research: '#9945FF',
  Shadow: '#8E9AAF',
};

interface GatekeeperBreakdownProps {
  routing: Record<string, number>;
}

export default function GatekeeperBreakdown({
  routing,
}: GatekeeperBreakdownProps) {
  const data = Object.entries(routing).map(([name, value]) => ({
    name,
    value,
  }));
