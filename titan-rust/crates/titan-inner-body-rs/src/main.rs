//! titan-inner-body-rs — Inner-body trinity daemon.
//!
//! Per SPEC §9.A `titan-inner-body-rs` row + master plan §10.5 chunk C5-2:
//! 7.83 Hz Schumann tick (G13 LOCKED biological constant), reads
//! sensor_cache_inner_body + inner_lower topology slice, applies
//! UNIFIED + LOCAL filter_down (multipliers from unified-spirit-rs +
//! inner-spirit-rs), applies ground_up (all 5D per G10), writes
//! inner_body_5d.bin (44 bytes total) content-hash gated, publishes
//! BODY_STATE (P1 coalesce-by-(src,type)) every tick.
//!
//! # Boot sequence (per SPEC §10.A B8 child side)
//!
//! 1. Parse CLI + env (TITAN_KERNEL_TITAN_ID / TITAN_BUS_SOCKET /
//!    TITAN_AUTHKEY_HEX / TITAN_KERNEL_SHM_DIR / TITAN_DAEMON_PARENT_PID).
//! 2. Init JSON logging (binary=titan-inner-body-rs, titan_id, pid).
//! 3. Set PR_SET_PDEATHSIG=SIGTERM (SPEC §11.C(1)).
//! 4. Connect to main bus + HMAC handshake + subscribe to REQUIRED set.
//! 5. Open kernel-created shm slots (sensor_cache + topology + inner_body).
//! 6. Send MODULE_READY.
//! 7. Enter tick loop @ 7.83 Hz; handle SIGTERM gracefully.
...
#[derive(Parser, Debug)]
#[command(version, about = "Titan Inner-Body Trinity Daemon")]
struct Cli {
    ...
    /// Data directory for disk-persistent state (e.g. checkpoint sidecars per
    /// §G5.2 item 4 + §24 sovereign-backup chain). Falls back to
    /// TITAN_KERNEL_DATA_DIR env, then "data" relative to cwd. D-SPEC-126.
...
async fn main() -> ExitCode {
    ...
    info!(
        binary = "titan-inner-body-rs",
        titan_id = %titan_id,
        pid = std::process::id(),
        bus_socket = ?bus_socket,
        shm_dir = ?shm_dir,
        event = "BOOT_START",
        "inner-body daemon boot start"
    );
    ...
    if let Err(e) = tick_loop::run(&bus_socket, &authkey, &shm_dir, &data_dir).await {
        error!(err = ?e, "inner-body daemon exited with error");
        return ExitCode::from(1);
    }

    info!(event = "SHUTDOWN_CLEAN", "inner-body daemon clean exit");
    ExitCode::SUCCESS
}
