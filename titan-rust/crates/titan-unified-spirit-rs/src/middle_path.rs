//! middle_path — Trinity middle-path loss function.
//!
//! Faithful port of `titan_plugin/logic/middle_path.py:51-107`. Used by
//! `FilterDownV5Engine::record_transition` (C4-3c) to compute reward from
//! the felt 130D vector at each transition.
//!
//! Per Python:
//! - `layer_loss(tensor)` = L2 distance from center (0.5).
//! - `middle_path_loss(body, mind, spirit, weights)` = weighted average
//!   of layer_losses, normalized to [0, 1] by theoretical max for 5-dim
...
/// Returns ≥ 0.0; 0.0 = perfect equilibrium. Max theoretical for 5-dim
/// tensor at extremes ≈ 1.118 (sqrt(5 × 0.25)).
...
mod tests {
    ...
    fn middle_path_loss_clamped_at_one() {
        // All extremes — theoretical max should clamp to 1.0
        let body = [0.0_f64; 5];
        let mind = [1.0_f64; 5];
