...
//!
//! Per SPEC §G5.2 item 5 + PLAN §1.5 / §6.6.2 + D-SPEC-128 (per-layer
//! extension, v1.58.3, 2026-05-25): the gain coefficients live in
...
/// Full sidecar payload: 3 layers × 32 bytes (D-SPEC-128).
    ...
    match std::fs::read(&path) {
        ...
        Ok(_short) => {
            warn!(
                ...
                reason = "sidecar payload < 96 bytes (D-SPEC-128 per-layer)",
                path = %path.display(),
                layer = ?layer,
...
mod tests {
    ...
    /// Build a full 96-byte D-SPEC-128 sidecar from per-layer gains.
    ...
    fn per_layer_slices_resolve_independently() {
        ...
        // every layer = the pre-D-SPEC-128 bug).
    ...
    fn short_sidecar_falls_back() {
        let dir = tempdir().unwrap();
        let path = dir.path().join(TRINITY_RESTORING_SIDECAR);
        // A pre-D-SPEC-128 32-byte sidecar is now considered too-short —
