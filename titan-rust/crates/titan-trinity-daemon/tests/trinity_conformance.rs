...
fn conformance_g5_2_gains_from_titan_params_toml() {
    ...
    // `titan_params.toml [trinity_restoring.{body,mind,spirit}]` per D-SPEC-128
    // per-layer schema, v1.58.3) and falls back to the kernel DEFAULT_* constants
    ...
    // Present + valid (D-SPEC-128, 96-byte per-layer sidecar) → values come
