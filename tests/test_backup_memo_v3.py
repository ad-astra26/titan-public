...
genesis anchor parse; mixed-mode chain decodable; malformed rejection.
...
def test_v3_memo_roundtrip_mode_a():
    key = v3.derive_backup_url_key(SEED)
    memo = v3.build_v3_memo(
        event_id=EVENT_ID, ts=1748000000, tier="PT",
...
def test_v3_memo_roundtrip_mode_b_byte_identical_url():
    memo = v3.build_v3_memo(
        event_id=EVENT_ID, ts=1748000001, tier="TC",
    ...
    assert parsed["prev"] == "5xPrevSig9aBcDe"[:16] or len(parsed["prev"]) <= 16
...
def test_prev_sig_truncated_to_16():
    memo = v3.build_v3_memo(
        event_id=EVENT_ID, ts=1, tier="SL", archive_hash=ARC, merkle_root=MRKL,
        arweave_tx=TX_DAY1, mode="B", prev_sig="A" * 88,  # full base58 sig length
    )
    parsed = v3.parse_v3_memo(memo)
...
def test_genesis_day1_anchor_parses():
    memo = v3.build_v3_memo(
        event_id="genesis-day1", ts=1744000140, tier="PT",
...
def test_mixed_mode_chain_decodable():
    key = v3.derive_backup_url_key(SEED)
    a = v3.build_v3_memo(event_id="e1", ts=1, tier="PT", archive_hash=ARC,
                         merkle_root=MRKL, arweave_tx=TX_PERSONALITY, mode="A",
                         prev_sig=None, url_key=key)
    b = v3.build_v3_memo(event_id="e2", ts=2, tier="TC", archive_hash=ARC,
                         merkle_root=MRKL, arweave_tx=TX_TIMECHAIN, mode="B",
...
def test_build_rejects_bad_inputs():
    ...
    with pytest.raises(ValueError):  # bad tier
        v3.build_v3_memo(event_id="e", ts=1, tier="ZZ", archive_hash=ARC,
                         merkle_root=MRKL, arweave_tx=TX_DAY1, mode="B")
    with pytest.raises(ValueError):  # bad mode
        v3.build_v3_memo(event_id="e", ts=1, tier="PT", archive_hash=ARC,
                         merkle_root=MRKL, arweave_tx=TX_DAY1, mode="C")
    with pytest.raises(ValueError):  # mode A without key
        v3.build_v3_memo(event_id="e", ts=1, tier="PT", archive_hash=ARC,
                         merkle_root=MRKL, arweave_tx=TX_DAY1, mode="A")
    with pytest.raises(ValueError):  # short hash
        v3.build_v3_memo(event_id="e", ts=1, tier="PT", archive_hash="abc",
                         merkle_root=MRKL, arweave_tx=TX_DAY1, mode="B")
    with pytest.raises(ValueError):  # event_id with delimiter
        v3.build_v3_memo(event_id="e;x", ts=1, tier="PT", archive_hash=ARC,
                         merkle_root=MRKL, arweave_tx=TX_DAY1, mode="B")
...
def test_memo_within_solana_cap():
    key = v3.derive_backup_url_key(SEED)
    memo = v3.build_v3_memo(event_id="f" * 36, ts=1748000000, tier="PT",
