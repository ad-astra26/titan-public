...
# ── headless inference supply (--inference-provider / --inference-key / env) ──

def test_resolve_inference_flag_hosted():
    prov, k, err = inference._resolve_inference_source("ollama_cloud", "K" * 30)
    assert (prov, k, err) == ("ollama_cloud", "K" * 30, "")


def test_resolve_inference_local_needs_no_key():
    prov, k, err = inference._resolve_inference_source("ollama_local", None)
    assert prov == "ollama_local" and k is None and err == ""


def test_resolve_inference_hosted_requires_key():
    prov, k, err = inference._resolve_inference_source("openrouter", None)
    assert prov is None and "needs a key" in err


def test_resolve_inference_unknown_provider():
    prov, k, err = inference._resolve_inference_source("groq", "x")
    assert prov is None and "unknown inference provider" in err


def test_resolve_inference_from_env(monkeypatch):
    monkeypatch.setenv("TITAN_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setenv("TITAN_INFERENCE_KEY", "sk-or-" + "z" * 12)
    prov, k, err = inference._resolve_inference_source(None, None)
    assert prov == "openrouter" and k.startswith("sk-or-") and err == ""


def test_resolve_inference_none_supplied(monkeypatch):
    monkeypatch.delenv("TITAN_INFERENCE_PROVIDER", raising=False)
    assert inference._resolve_inference_source(None, None) == (None, None, "")


def _inference_repo(tmp_path: Path) -> Path:
    (tmp_path / "titan_hcl").mkdir()
    (tmp_path / "titan_hcl" / "config.toml").write_text(
        "[inference]\ninference_provider = \"venice\"\n")
    return tmp_path
...
def test_run_inference_phase_headless_wires_hosted(tmp_path: Path):
    root = _inference_repo(tmp_path)
    sp = tmp_path / "secrets.toml"
    res = inference.run_inference_phase(default=False, install_root=root, secrets_path=sp,
                                        provider="ollama_cloud", key="K" * 30)
    assert any(r.severity == "ok" for r in res)
    import tomllib as _t
    assert _t.load(open(sp, "rb"))["inference"]["ollama_cloud_api_key"] == "K" * 30
    cfg = {e.dotted: e.raw_value for e in cm.parse_toml_with_comments(root / "titan_hcl" / "config.toml")}
    assert cfg["inference.inference_provider"] == '"ollama_cloud"'


def test_run_inference_phase_headless_missing_key_fails(tmp_path: Path):
    root = _inference_repo(tmp_path)
    res = inference.run_inference_phase(default=False, install_root=root,
                                        secrets_path=tmp_path / "s.toml", provider="openrouter")
    assert res[0].severity == "fail" and "needs a key" in res[0].detail


# ── --simulate install wiring (mainnet-readiness rehearsal, #34/G7) ───────────

def test_genesis_phase_simulate_builds_ceremony_simulate_cmd(tmp_path: Path, monkeypatch):
    (tmp_path / "scripts").mkdir()
    (tmp_path / "scripts" / "genesis_ceremony.py").write_text("# stub")
    venv_py = tmp_path / "venv" / "bin" / "python"
    venv_py.parent.mkdir(parents=True)
    venv_py.write_text("#!/bin/sh\n")
    captured = {}
    monkeypatch.setattr(genesis_runner.subprocess, "check_call",
                        lambda cmd, cwd=None: captured.update(cmd=cmd) or 0)
    res = genesis_runner.run_genesis_phase(tmp_path, Mode.MAINNET, venv_python=venv_py, simulate=True)
    cmd = captured["cmd"]
    assert "--simulate" in cmd and "--network" in cmd and "mainnet" in cmd
    assert "--keep-plaintext" not in cmd and "--skip-onchain" not in cmd
    assert res[0].severity == "ok" and "SIMULATED" in res[0].detail
    # a rehearsal mints/burns nothing → no bootable identity is materialized
    assert not genesis_runner.keypair_path(tmp_path).exists()


def test_install_parser_accepts_simulate_and_inference_flags():
    args = build_parser().parse_args(
        ["install", "--mode", "mainnet", "--simulate",
         "--inference-provider", "ollama_cloud", "--inference-key", "K" * 30])
    assert args.simulate is True
    assert args.inference_provider == "ollama_cloud" and args.inference_key == "K" * 30


def test_install_parser_rejects_unknown_inference_provider():
    with pytest.raises(SystemExit):
        build_parser().parse_args(["install", "--inference-provider", "groq"])
