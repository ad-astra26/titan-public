...
# ── pyproject pin-drift guard (INV-PROV-10 / gate G8 offline) ─────────────────

def test_research_pyproject_pins_match_t1_set():
    """The [research] extra is ==-pinned to T1's live venv (§6); html2text is in
    BASE (load-bearing for the default fast scrape, not behind --minimal)."""
    import tomllib
    from pathlib import Path
    root = Path(__file__).resolve().parents[1]
    with open(root / "pyproject.toml", "rb") as fh:
        cfg = tomllib.load(fh)
    base = cfg["project"]["dependencies"]
    research = cfg["project"]["optional-dependencies"]["research"]

    assert "html2text==2025.4.15" in base
    assert not any("html2text" in r for r in research)   # moved out of the extra
    assert set(research) == {
        "crawl4ai==0.8.0",
        "playwright==1.58.0",
        "tf-playwright-stealth==1.2.0",
        "unstructured[pdf,docx,pptx,xlsx]==0.21.5",
        "unstructured-inference==1.5.2",
        "onnxruntime==1.22.1",
        "nltk==3.9.3",
        "ollama==0.6.1",
    }
