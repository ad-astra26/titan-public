...
def test_cleanup_adopted_dead_pid_is_noop():
    ...
    with patch("titan_hcl.guardian_hcl.os.kill") as m_kill:
...
def test_cleanup_adopted_escalates_to_sigkill_after_grace():
    ...
    with patch("titan_hcl.guardian_hcl.os.kill", side_effect=fake_kill), \
         patch("titan_hcl.guardian_hcl.time.sleep"), \
         patch("titan_hcl.guardian_hcl.time.time", side_effect=[0, 0.5, 1.0, 1.5, 2.5]):
