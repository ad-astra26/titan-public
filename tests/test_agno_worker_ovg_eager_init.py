"""
D-SPEC-137 regression test — OutputVerifier eager-init at agno_worker boot.

Pre-D-SPEC-137: `worker_plugin._output_verifier` was a `@property` that
...
D-SPEC-137 fix: `agno_worker.agno_worker_main` accesses
...
def test_ovg_is_materialized_at_boot_path():
    ...
    `_local_ovg_instance` attribute. This pins the D-SPEC-137 invariant:
    ...
    assert hasattr(wp, "_local_ovg_instance"), (
        "D-SPEC-137: eager-init line in agno_worker_main MUST trigger "
...
def test_agno_worker_main_runs_eager_init_after_construct():
    ...
    assert "_output_verifier" in source, (
        "D-SPEC-137: `agno_worker_main` MUST contain a `_output_verifier` "
    ...
    assert "worker_plugin is not None" in source, (
        "D-SPEC-137: eager OVG init MUST guard against failed "
        "WorkerPlugin construction (`worker_plugin is not None`)."
    )

    # Required pattern: the log message must include "D-SPEC-137" so the
    # boot-time gate is identifiable in journalctl. This is the live
    # verification signal we use post-deploy.
    assert "D-SPEC-137" in source, (
        "D-SPEC-137: eager-init log line MUST include the D-SPEC tag — "
...
def test_agno_worker_main_eager_init_does_not_raise_on_property_error(caplog):
    ...
    assert "_ = worker_plugin._output_verifier" in source, (
        "D-SPEC-137: canonical eager-init invocation missing."
    )
    assert "except Exception as _ovg_err:" in source, (
        "D-SPEC-137: eager-init MUST be wrapped in try/except so a "
        "property-side failure does not crash boot (defense-in-depth "
        "for lazy-retry fallback)."
    )
    assert "OVG eager-init failed" in source, (
        "D-SPEC-137: failure-path log MUST be present + recognisable in "
