...
# ── B3 sibling: outer_inner_bridge ───────────────────────────────────────

def _build_oinb(tmp_path, *, author, relevance, is_following):
    ...
    ce = sqlite3.connect(et)
    ce.execute("CREATE TABLE felt_experiences (id INTEGER PRIMARY KEY AUTOINCREMENT, "
               "titan_id TEXT, author TEXT, topic TEXT, relevance REAL, felt_summary TEXT, "
               "concept_signals TEXT, semantic_concepts TEXT, created_at REAL)")
    ce.execute("INSERT INTO felt_experiences (titan_id, author, topic, relevance, "
               "felt_summary, concept_signals, semantic_concepts, created_at) "
               "VALUES ('T1', ?, 'sovereignty', ?, 'a sharp take on custody', '[]', '[]', ?)",
               (author, relevance, now - 3600))
    ce.commit(); ce.close()
    cg = sqlite3.connect(sg)
    cg.execute("CREATE TABLE community_registry (user_name TEXT PRIMARY KEY, bio TEXT, "
               "is_following INTEGER, last_tweet_text TEXT, last_tweet_id TEXT)")
    if is_following:
        cg.execute("INSERT INTO community_registry (user_name, bio, is_following, "
                   "last_tweet_text, last_tweet_id) VALUES (?, 'og', 1, 'their tweet', 'TW42')",
                   (author,))
    cg.commit(); cg.close()
    from titan_hcl.logic.social_x.archetypes.outer_inner_bridge import OuterInnerBridgeArchetype
    return OuterInnerBridgeArchetype(gateway=None, social_x_db_path=sx,
                                     events_teacher_db=et, social_graph_db=sg,
                                     inner_memory_db=str(tmp_path / "im.db"))


def test_b3_oinb_engages_nonfollowed_high_relevance(tmp_path):
    oinb = _build_oinb(tmp_path, author="nightskynow", relevance=0.9, is_following=0)
    rows = oinb._fetch_fresh_outer(titan_id="T1", now=time.time(), cited=set())
    assert any(r["author"] == "nightskynow" and r["tweet_id"] == "" for r in rows)


def test_b3_oinb_excludes_nonfollowed_low_relevance(tmp_path):
    oinb = _build_oinb(tmp_path, author="randomguy", relevance=0.6, is_following=0)
    rows = oinb._fetch_fresh_outer(titan_id="T1", now=time.time(), cited=set())
    assert not any(r["author"] == "randomguy" for r in rows)


def test_b3_oinb_followed_quote_tweets(tmp_path):
    oinb = _build_oinb(tmp_path, author="lopp", relevance=0.6, is_following=1)
    rows = oinb._fetch_fresh_outer(titan_id="T1", now=time.time(), cited=set())
    match = [r for r in rows if r["author"] == "lopp"]
    assert match and match[0]["tweet_id"] == "TW42"
