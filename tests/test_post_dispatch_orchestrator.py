...
# ── C2 (INV-XEFF-4): adaptive mention-poll backoff ────────────────────

class _SeqMentionGateway(_FakeGateway):
    """discover_mentions returns a scripted sequence (one list per ACTUAL
    poll — gated calls don't consume the sequence)."""
...
def _backoff_cfg(enabled: bool = True) -> dict:
    return {
        "twitter_social": {}, "inference": {}, "stealth_sage": {},
        "social_x": {"replies": {
            "mention_check_cooldown_seconds": 1800.0,
            "mention_backoff_cap_seconds": 7200.0,
            "mention_backoff_enabled": enabled,
            "max_replies_per_cycle": 3,
        }},
    }


def test_mention_backoff_widens_on_empty_streak():
    """Empty polls double the effective cooldown — a poll that would fire
    under the base interval is gated once the streak grows."""
    gw = _SeqMentionGateway([[], [], []])  # all empty
    orch, _, _, _ = _mk(gateway=gw)
    cfg = _backoff_cfg()
    now0 = 1_000_000.0
    orch._run_mention_cycle(full_config=cfg, grounded_words=[], neuromods={},
                            emotion="wonder", now=now0)        # fires → streak 1
    assert orch._mention_empty_streak == 1
    # +1800 would fire under the base, but effective is now 3600 → gated
    r = orch._run_mention_cycle(full_config=cfg, grounded_words=[],
                                neuromods={}, emotion="wonder", now=now0 + 1800)
    assert r == 0
    assert orch._mention_empty_streak == 1     # gated → discover not called
    # +3600 → past the widened interval → fires → streak 2
    orch._run_mention_cycle(full_config=cfg, grounded_words=[], neuromods={},
                            emotion="wonder", now=now0 + 3600)
    assert orch._mention_empty_streak == 2


def test_mention_backoff_resets_on_discovery():
    """Any discovered mention snaps the streak back to 0 (responsive again)."""
    mention = [{"tweet_id": "m1", "titan_id": "T1", "text": "hi",
                "author_handle": "a", "relevance_score": 0.5}]
    gw = _SeqMentionGateway([[], [], mention])
    orch, _, _, _ = _mk(gateway=gw)
    cfg = _backoff_cfg()
    now0 = 1_000_000.0
    orch._run_mention_cycle(full_config=cfg, grounded_words=[], neuromods={},
                            emotion="wonder", now=now0)            # streak 1
    orch._run_mention_cycle(full_config=cfg, grounded_words=[], neuromods={},
                            emotion="wonder", now=now0 + 3600)     # streak 2
    assert orch._mention_empty_streak == 2
    # streak 2 → effective min(1800*4, 7200)=7200; poll past it finds a mention
    orch._run_mention_cycle(full_config=cfg, grounded_words=[], neuromods={},
                            emotion="wonder", now=now0 + 3600 + 7200)
    assert orch._mention_empty_streak == 0


def test_mention_backoff_capped():
    """Effective cooldown never exceeds the cap regardless of streak size."""
    gw = _SeqMentionGateway([[]] * 5)
    orch, _, _, _ = _mk(gateway=gw)
    cfg = _backoff_cfg()
    now0 = 1_000_000.0
    orch._mention_empty_streak = 8          # 1800*2^8 ≫ cap → clamps to 7200
    orch._last_mention_check_ts = now0
    r_gated = orch._run_mention_cycle(full_config=cfg, grounded_words=[],
                                      neuromods={}, emotion="wonder",
                                      now=now0 + 7000)   # < cap
    assert r_gated == 0
    r_fire = orch._run_mention_cycle(full_config=cfg, grounded_words=[],
                                     neuromods={}, emotion="wonder",
                                     now=now0 + 7300)    # ≥ cap → fires
    assert r_fire >= 0
    assert orch._mention_empty_streak == 9   # incremented (still empty), bounded


def test_mention_backoff_disabled_uses_base():
    """enabled=false → always the base interval, ignoring the streak."""
    gw = _SeqMentionGateway([[]] * 3)
    orch, _, _, _ = _mk(gateway=gw)
    cfg = _backoff_cfg(enabled=False)
    now0 = 1_000_000.0
    orch._mention_empty_streak = 5           # would widen if backoff were on
    orch._last_mention_check_ts = now0
    r = orch._run_mention_cycle(full_config=cfg, grounded_words=[],
                                neuromods={}, emotion="wonder",
                                now=now0 + 1800)   # exactly the base → fires
    assert r >= 0
    assert orch._mention_empty_streak == 6   # poll happened (empty) → +1
