...
def test_present_anchor_via_avm_binary(monkeypatch, tmp_path):
    # anchor is verified against the deterministic avm-installed binary
    # (~/.avm/bin/anchor-<version>), NOT the racy shim that misreports right
    # after `avm use` (the 2026-06-03 T4 false-negative).
    avm_bin = tmp_path / ".avm" / "bin" / "anchor-0.32.1"
    avm_bin.parent.mkdir(parents=True)
    avm_bin.write_text("#!/bin/sh\n")
    monkeypatch.setattr(tc.Path, "home", classmethod(lambda cls: tmp_path))
    monkeypatch.setattr(tc, "_probe_version",
                        lambda exe, version_arg="--version":
                        "anchor-cli 0.32.1" if exe == str(avm_bin) else "anchor-cli 2.3.0")
    st = tc.tool_present_at("anchor", "0.32.1")
    assert st.present and st.compatible and st.found == "0.32.1"


def test_anchor_avm_binary_wrong_version_incompatible(monkeypatch, tmp_path):
    avm_bin = tmp_path / ".avm" / "bin" / "anchor-0.32.1"
    avm_bin.parent.mkdir(parents=True)
    avm_bin.write_text("x")
    monkeypatch.setattr(tc.Path, "home", classmethod(lambda cls: tmp_path))
    monkeypatch.setattr(tc, "_probe_version",
                        lambda exe, version_arg="--version": "anchor-cli 0.31.0")
    st = tc.tool_present_at("anchor", "0.32.1")
    assert st.present and not st.compatible


def test_anchor_falls_back_to_path_shim_without_avm(monkeypatch, tmp_path):
    # no avm-managed binary → generic PATH-shim probe (non-avm installs)
    monkeypatch.setattr(tc.Path, "home", classmethod(lambda cls: tmp_path))
