...
def test_satisfies_meta_delegate_reasoning_engine_interface():
    """The OuterMeta DELEGATE seam (meta_reasoning.py _prim_delegate +
    _check_delegate) drives `reasoning_engine` via this exact surface — the
    OuterReasoningEngine must be a drop-in. (gut_agreement is the one the
    inner ReasoningEngine has that a naive outer build would miss.)"""
    eng = OuterReasoningEngine()
    assert callable(eng.set_strategy_bias)
    assert callable(eng.clear_strategy_bias)
    assert callable(eng.run_chain)
    assert isinstance(eng._total_chains, int)       # _prim_delegate reads it
    assert isinstance(eng.confidence, float)        # _check_delegate reads it
    assert isinstance(eng.gut_agreement, float)     # _check_delegate reads it
    assert eng.gut_agreement == 0.5                 # neutral (no gut coupling)
